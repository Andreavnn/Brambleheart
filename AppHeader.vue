...
const headerCharacterFlipped=computed(()=>route.name==='characters'||route.name==='rules')
...
<template>
  <header class="app-header-wrap bramble-site-header">
    ...
    <PrimaryNav />
    <img v-if="headerCharacter" :src="headerCharacter" class="page-header-character" :class="{flipped:headerCharacterFlipped}" alt="" aria-hidden="true" />
  </header>
</template>
...
@media(max-width:680px){.header-control-row{width:min(760px,calc(100vw - 20px))}.game-updates-header-link{bottom:5px;font-size:calc(8.5px + var(--font-offset))}.page-header-character{right:4px;bottom:-114px;width:124px;height:106px}}
@media(max-width:430px){.page-header-character{width:106px;height:94px;bottom:-100px}}
</style>
