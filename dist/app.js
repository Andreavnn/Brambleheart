...
function header() {
  ...
  return `<header class="app-header-wrap">
    <div class="wip-banner">Bramble Heart is a work in progress. v0.01 reflects the current uploaded rules and preserves unresolved source terminology.</div>
    <div class="app-header">
      <span class="header-spacer"></span>
      <a data-route href="/characters" class="brand-stack brand" aria-label="Bramble Heart Character List">
        <img class="brand-logo" src="/assets/Logo.png" alt="Brambleheart — Small Heroes, Big Adventures" />
...
function shell(content) {
  $app.innerHTML = `<div class="app-shell"><main class="page">${header()}${content}</main><footer class="app-footer"><img src="/assets/Icon.png" alt="" aria-hidden="true" /><span>Brambleheart · Alpha Build ${BUILD} · Local-first prototype</span></footer></div>`
...
function characterSummary(character) {
  ...
  return `<article class="saved-card card-surface" data-character="${character.id}">
    ...
    <div class="character-detail" hidden id="detail-${character.id}">
      ...
      <div class="derived-note"><strong>Reliable v0.01 derived values:</strong> Mettle ${character.attributes.might*2}, Ward ${character.attributes.hide*2}, Control ${character.attributes.lore*2}, Power ${character.attributes.might}, Guts ${character.attributes.hide}. <span>Speed is not auto-derived because current source chapters use both Agility and Prowess terminology.</span></div>
...
function renderCharacters() {
  ...
  shell(`${titleBlock('CHARACTERS','Character List','Create and manage Bramble Heart characters on this device. v0.01 stores characters locally and can export them as JSON.')}
...
function renderCharacterForm() {
  ...
  host.innerHTML = `<section class="form-card character-form"><div class="form-heading"><div><p class="eyebrow">NEW CHARACTER</p><h2>Create Character</h2></div><button class="icon-button" id="close-character" aria-label="Close">×</button></div>
    ...
    <div class="source-note">Species trait details and Talent selection are intentionally deferred in v0.01 because their full structured data has not yet been normalized from the uploaded layouts.</div>
...
function renderRules() {
  shell(`${titleBlock('REFERENCE','Rules','Browse the current Bramble Heart rule structure. Search works across chapter titles, summaries, and loaded v0.01 rule text.')}
...
function renderSettings() {
  shell(`${titleBlock('PREFERENCES','Settings','Display preferences are stored on this device. Character data remains local unless you export it.')}
    ...
    <section class="settings-card"><div class="setting-row"><div><strong>Local Character Data</strong><small>Delete all characters stored by this v0.01 build.</small></div><button class="danger-button" id="reset-characters">Reset Characters</button></div><div class="setting-row"><div><strong>Reset App</strong><small>Restore settings and clear all locally stored Bramble Heart v0.01 data.</small></div><button class="danger-button" id="reset-all">Reset All</button></div></section>
  ...
  document.querySelector('#reset-all').onclick=()=>{if(confirm('Reset all Bramble Heart v0.01 local data?')){localStorage.removeItem(STORE.characters);localStorage.removeItem(STORE.settings);settings={theme:'system',text:'medium',compact:false};applySettings();renderSettings()}}
}
