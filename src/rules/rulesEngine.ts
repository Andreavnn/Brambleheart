...
export type ProtectiveEquipmentSource={name:string;detail?:string;category?:string;equipped?:boolean;attachedTo?:string;choice?:string}
...
export function equipmentResistanceBonus(items:ProtectiveEquipmentSource[]|undefined,damageType:string){return equippedProtectiveItems(items).some(item=>canonicalGearName(item.name)==='Durtlehide'&&item.choice===damageType)?2:0}
...
export function equipmentSpellManaReduction(items:TrinketAwareEquipmentSource[]|undefined,spellName:string){return equippedTrinketGear(items).some(item=>canonicalGearName(item.name)==='Scriptweave Book'&&item.attachedTo===spellName)?1:0}
...
/** Once-per-round bonus Health restored by one healing Spell from equipped trinkets. */
export function equipmentHealingSpellBonus(items:ProtectiveEquipmentSource[]|undefined){return equippedTrinketGear(items).some(item=>canonicalGearName(item.name)==='Driftwood Charm')?1:0}
...
export function equipmentAttachmentTargets(item:ProtectiveEquipmentSource,equipment:ProtectiveEquipmentSource[]|undefined,spellTargets:readonly string[]=[]){const name=canonicalGearName(item.name),weapons=(equipment||[]).filter(candidate=>candidate.category==='Weapon');if(name==='Quickdraw Quiver')return weapons.filter(candidate=>/\bbow\b/i.test(candidate.name)&&!/crossbow/i.test(candidate.name)).map(candidate=>candidate.name);if(name==='Featherwind Bolt-Case')return weapons.filter(candidate=>/crossbow/i.test(candidate.name)).map(candidate=>candidate.name);if(name==='Wristloop')return weapons.filter(candidate=>/Thrown/i.test(candidate.detail||'')).map(candidate=>candidate.name);if(name==='Journey Knot'||name==='Sharpening Stone')return weapons.map(candidate=>candidate.name);if(name==='Heartward Token')return(equipment||[]).filter(candidate=>candidate.category==='Armor & Shield'&&protectiveGearKind(candidate)==='armor').map(candidate=>candidate.name);if(name==='Scriptweave Book')return Array.from(new Set(spellTargets.filter(Boolean)));return[]}

export const CHARACTER_MAX_HEALTH=30
export function healthCondition(currentHealth:number|undefined){const value=Math.max(0,Math.min(CHARACTER_MAX_HEALTH,Math.floor(Number(currentHealth??CHARACTER_MAX_HEALTH)||0)));if(value<=0)return{label:'Defeated',condition:-3};if(value<=2)return{label:'Last Breath',condition:-3};if(value<=4)return{label:'Critical',condition:-2};if(value<=7)return{label:'Wounded',condition:-1};return{label:'Healthy',condition:0}}
export function equipmentWeightLb(item:{detail?:string;weightLb?:number;quantity?:number}|undefined){if(!item)return 0;const direct=Number(item.weightLb);let weight=Number.isFinite(direct)&&direct>=0?direct:0;if(!weight){const text=String(item.detail||'').replace(/¼/g,'.25').replace(/½/g,'.5').replace(/¾/g,'.75');const match=text.match(/(?:Weight\s*)?([0-9]+(?:\.[0-9]+)?)\s*lb\.?/i);weight=match?Number(match[1]):0}return weight*Math.max(1,Math.floor(Number(item.quantity)||1))}
...
const STRUCTURED_RULE_LABELS=['COST','TRIGGER','DECLARE','DECLEAR','TARGET','AREA','STRIKE','TO HIT','ATTRIBUTE SAVE','SAVE','HEX','ON FAILURE','ON SUCCESS','EFFECT','SLOSH','SEEPING SLOSH','CINDERSTEP','FORMLESS','SECOND CAST','DAMAGE','LONG RANGE','PICK UP','AFTERBURN','PURIFY','EMPOWER','IMPACT','DURATION','OPEN DEFENCE','ARMOR MASTERY','DEFENSELESS','FORTIFIED','RAPID ATTACK','RESTICTION','RESTICTIONS','RESTRICTION','RESTRICTIONS','REQUIRES','COOLDOWN','EXAMPLE','KEYWORDS'] as const
const RULE_FIELD_ORDER=['COST','TRIGGER','TARGET','AREA','TO HIT','SAVE','HEX','ON FAILURE','ON SUCCESS','EFFECT','SLOSH','SEEPING SLOSH','CINDERSTEP','FORMLESS','SECOND CAST','DAMAGE','IMPACT','LONG RANGE','PICK UP','AFTERBURN','PURIFY','EMPOWER','DURATION','OPEN DEFENCE','ARMOR MASTERY','DEFENSELESS','FORTIFIED','RAPID ATTACK','COOLDOWN','RESTRICTIONS','REQUIRES','EXAMPLE','KEYWORDS'] as const
function canonicalRuleLabel(value:string){const label=value.toUpperCase();if(label==='DECLEAR'||label==='DECLARE')return'TARGET';if(label==='STRIKE')return'TO HIT';if(label==='ATTRIBUTE SAVE')return'SAVE';if(label==='RESTICTION'||label==='RESTICTIONS'||label==='RESTRICTION')return'RESTRICTIONS';return label}
