...
.page-header-character-stage{position:absolute;inset-block:0;left:50%;width:100vw;transform:translateX(-50%);overflow:hidden;pointer-events:none;z-index:2}
.page-header-character{position:absolute;right:12px;bottom:8px;width:clamp(118px,22vw,180px);height:126px;object-fit:contain;object-position:right bottom;pointer-events:none;filter:drop-shadow(0 3px 2px rgba(0,0,0,.08))}
.page-header-character.flipped{object-position:left bottom;transform:scaleX(-1)}
@media(max-width:680px){.header-control-row{width:min(760px,calc(100vw - 20px))}.game-updates-header-link{bottom:5px;font-size:calc(8.5px + var(--font-offset))}.page-header-character{right:6px;bottom:6px;width:124px;height:106px}}
@media(max-width:430px){.page-header-character{right:4px;bottom:4px;width:106px;height:94px}}
</style>
