...
.brand-header-stage{position:relative;display:flow-root}
...
.page-header-character{position:absolute;z-index:2;right:12px;bottom:12px;width:clamp(118px,22vw,180px);height:126px;object-fit:contain;object-position:right bottom;pointer-events:none;filter:drop-shadow(0 3px 2px rgba(0,0,0,.08))}
.page-header-character.flipped{object-position:left bottom;transform:scaleX(-1)}
@media(max-width:680px){.header-control-row{width:min(760px,calc(100vw - 20px))}.game-updates-header-link{bottom:5px;font-size:calc(8.5px + var(--font-offset))}.page-header-character{right:4px;bottom:8px;width:124px;height:106px}}
@media(max-width:430px){.page-header-character{width:106px;height:94px;bottom:6px}}
</style>
