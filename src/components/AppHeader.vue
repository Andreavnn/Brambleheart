...
<template>
  <header class="app-header-wrap bramble-site-header">
    ...
    <div class="app-header" :class="{compact}"><div class="header-control-row"><button v-if="backTo" type="button" class="icon-button back-button" :aria-label="backLabel||'Back'" @click="goBack"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.5 5 8.5 12l7 7"/></svg></button><div v-else class="header-spacer"></div><button class="icon-button theme-button" type="button" :aria-label="darkMode?'Switch to light mode':'Switch to dark mode'" @click="toggleTheme"><svg v-if="darkMode" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.42 1.42M17.65 17.65l1.42 1.42M2 12h2M20 12h2M4.93 19.07l1.42-1.42M17.65 6.35l1.42-1.42"/></svg><svg v-else viewBox="0 0 24 24" aria-hidden="true"><path d="M20.2 15.4A8.5 8.5 0 0 1 8.6 3.8 8.5 8.5 0 1 0 20.2 15.4Z"/></svg></button></div><div class="brand-stack brand-update-stack"><RouterLink to="/news" class="brand-logo-link" aria-label="Brambleheart News"><img src="/assets/Logo.png" alt="Brambleheart — Small Heroes, Big Adventures" class="brand-logo" /></RouterLink><RouterLink to="/game-updates" class="game-updates-header-link">Rule Updates v{{ GAME_RULES_VERSION }}</RouterLink></div></div>
    <PrimaryNav />
    <img v-if="headerCharacter" :src="headerCharacter" class="page-header-character" alt="" aria-hidden="true" />
  </header>
</template>

<style scoped>
.bramble-site-header{position:relative}.header-control-row{grid-column:1/-1;grid-row:1;z-index:3;width:min(760px,calc(100vw - 28px));justify-self:center;display:flex;align-items:center;justify-content:space-between;pointer-events:none}.header-control-row>*{pointer-events:auto}.header-control-row .header-spacer{pointer-events:none}.brand-update-stack{grid-column:1/-1;grid-row:1;z-index:1;display:grid;justify-items:center;align-content:center;gap:2px;min-width:0;height:100%;position:relative}.brand-logo-link{display:block;min-width:0}.game-updates-header-link{position:absolute;left:50%;bottom:8px;transform:translateX(-50%);width:max-content;max-width:calc(100% - 24px);color:var(--ink-soft);font-size:calc(9px + var(--font-offset));font-weight:850;letter-spacing:.055em;text-transform:uppercase;text-align:center;text-decoration:none}.game-updates-header-link:hover,.game-updates-header-link.router-link-active{color:var(--accent-dark);text-decoration:underline;text-underline-offset:3px}.page-header-character{position:absolute;z-index:4;right:12px;bottom:-126px;width:clamp(118px,22vw,180px);height:126px;object-fit:contain;object-position:right bottom;pointer-events:none;filter:drop-shadow(0 3px 2px rgba(0,0,0,.08))}@media(max-width:680px){.header-control-row{width:min(760px,calc(100vw - 20px))}.game-updates-header-link{bottom:5px;font-size:calc(8.5px + var(--font-offset))}.page-header-character{right:4px;bottom:-104px;width:124px;height:106px}}@media(max-width:430px){.page-header-character{width:106px;height:94px;bottom:-92px}}
</style>
