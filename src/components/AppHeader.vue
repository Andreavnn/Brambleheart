...
const headerCharacterClass=computed(()=>({flipped:route.name==='characters'||route.name==='rules','page-header-character--simulator':route.name==='simulator'}))
...
.page-header-character{position:absolute;z-index:4;right:12px;bottom:-138px;width:clamp(118px,22vw,180px);height:126px;object-fit:contain;object-position:right bottom;pointer-events:none;filter:drop-shadow(0 3px 2px rgba(0,0,0,.08))}
.page-header-character.flipped{transform:scaleX(-1)}
.page-header-character--simulator{bottom:-128px}
@media(max-width:680px){.header-control-row{width:min(760px,calc(100vw - 20px))}.game-updates-header-link{bottom:5px;font-size:calc(8.5px + var(--font-offset))}.page-header-character{right:4px;bottom:-114px;width:124px;height:106px}.page-header-character--simulator{bottom:-104px}}
@media(max-width:430px){.page-header-character{width:106px;height:94px;bottom:-100px}.page-header-character--simulator{bottom:-90px}}
</style>
