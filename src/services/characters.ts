...
export function normalizeCharacterRecord(record:CharacterRecord):CharacterRecord{
  ...
  return{...record,equipment,adventureKit:record.adventureKit!==false,currentHealth:Math.max(0,Math.min(30,Math.floor(Number(record.currentHealth??30)||0))),skills,pathSkills,skillRanks,cultureSkillChoices,talents:Array.from(new Set((record.talents||[]).map(canonicalTalentName).filter(name=>!RETIRED_TALENTS.has(name)))),spells:normalizeLoreSpellSelection(record.spells),startingWealthWp,wealthWp,currencyAddedWp,startingWealth:startingWealthWp/WP_PER_NP,wealthRemaining:wealthWp/WP_PER_NP,wealthCurrency:'NP',currencyAddedNp:currencyAddedWp/WP_PER_NP,creationComplete,status,draft:!creationComplete,locked:Boolean(record.locked)}
}
...
function seedPremadeCharacters(characters:CharacterRecord[]){
  if(readLocalStorage(STORAGE_KEYS.premadeCharactersSeeded)==='1')return characters
  const existingIds=new Set(characters.map(character=>character.id))
  const additions=premadeCharacters.filter(character=>!existingIds.has(character.id)).map(character=>normalizeCharacterRecord(JSON.parse(JSON.stringify(character)) as CharacterRecord))
  const seeded=[...additions,...characters]
  const saved=writeLocalStorage(CHARACTER_STORE,JSON.stringify(seeded))
  if(!saved.ok)return characters
  writeLocalStorage(STORAGE_KEYS.premadeCharactersSeeded,'1')
  return seeded
}
export function loadCharacters():CharacterRecord[]{try{const parsed=JSON.parse(readLocalStorage(CHARACTER_STORE)||'[]');if(!Array.isArray(parsed))return[];const normalized=(parsed as CharacterRecord[]).map(normalizeCharacterRecord);if(JSON.stringify(parsed)!==JSON.stringify(normalized))writeLocalStorage(CHARACTER_STORE,JSON.stringify(normalized));return seedPremadeCharacters(normalized)}catch{return[]}}
