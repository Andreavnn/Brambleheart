export interface CoreActionField {
  label:string
  value:string
}

export interface CoreActionDefinition {
  name:string
  flavor:string
  cost?:string
  fields:CoreActionField[]
  keywords:string[]
}

/**
 * Canonical shared Core Actions.
 *
 * Core Actions are the only roots of an Ability Chain. Abilities may modify or
 * trigger from a Core Action and from later Ability resolutions in that chain.
 */
export const coreActions:CoreActionDefinition[]=[
  ...
  {name:'STRIDE',flavor:'The forest favors those who move with purpose.',fields:[{label:'TRIGGER',value:'During your turn.'},{label:'EFFECT',value:'Move up to your Speed in squares. This movement may be split before and after your Core Combat Action.'}],keywords:['CORE','MOVE']},
  ...
  {name:'ARCANE COMMAND',flavor:'To speak the words of power is to bend the rhythm of the world.',cost:'See Spell Details',fields:[{label:'TRIGGER',value:'During your turn.'},{label:'DECLARE',value:'Choose [1] non-Signature spell you know.'},{label:'EFFECT',value:'Pay the spell’s Mana cost and resolve its Spell Details.'},{label:'SECOND CAST',value:'Once per turn, before or after resolving a Core Combat Action, you may perform Arcane Command. This performance must be used to cast an Enhance or Hex Spell. Increase that Spell’s Mana cost by [+1].'},{label:'RESTRICTIONS',value:'You must meet all requirements stated by the chosen spell.'}],keywords:['CORE','COMBAT','MAGIC']},
  {name:'REACTION',flavor:'An opening appears; instinct answers before thought can follow.',fields:[{label:'TRIGGER',value:'When an Ability you possess with the Reactive keyword meets its printed Trigger.'},{label:'EFFECT',value:'Choose [1] eligible Reactive Ability and resolve it.'},{label:'RESTRICTIONS',value:'You may use the Reaction Core Action once per round. Resolving a Reactive Ability spends your Reaction Core Action for that round.'}],keywords:['CORE','REACTION']},
  {name:'RENEW THE HEART',flavor:'The will to rise again is its own kind of magic.',fields:[{label:'TRIGGER',value:'At the start of your turn, or when an effect Compels you to Renew the Heart.'},{label:'EFFECT',value:'Choose one ongoing effect currently affecting you, or the specific effect that Compelled this action, and make the required Attribute Save.'},{label:'SAVE',value:'Roll (3d10) + Heart + condition(s). Use the passive target stated by the effect; if none is stated, use Medium.'}],keywords:['CORE','PASSIVE']},
]

/** Legacy type/export aliases for older imports and external custom code. One data list remains authoritative. */
export type CoreAbilityField=CoreActionField
export type CoreAbilityDefinition=CoreActionDefinition
export const coreAbilities=coreActions
