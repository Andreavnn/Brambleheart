...
import { canEquipProtectiveEquipment, canEquipTrinketInArmorSlot, characterStatus, characterWealthWp, loadCharacters, setActiveArcaneFocus, setProtectiveEquipmentEquipped, setTrinketEquipmentEquipped, writeCharacters, type CharacterRecord, type PurchasedEquipment } from '../services/characters'
...
function purchaseGear(){
  ...
  const hasDedicatedTrinket=(c.equipment||[]).some(existing=>isTrinketGear(existing)&&existing.equipped===true&&existing.trinketSlot!=='armor')
  const equipped=kind?!((c.equipment||[]).some(existing=>protectiveGearKind(existing)===kind&&existing.equipped===true)):trinket?!hasDedicatedTrinket:undefined
...
function equipTrinket(index:number,slot:'trinket'|'armor'){
  ...
  const item=character.value?.equipment?.[index]
  message.value=item?.equipped?`${item.name} equipped in the ${item.trinketSlot==='armor'?'Armor & Shield':'Trinket'} slot.`:`${current.name} unequipped.`
}
