...
import { canEquipProtectiveEquipment, canEquipTrinketInShieldSlot, characterStatus, characterWealthWp, loadCharacters, setActiveArcaneFocus, setProtectiveEquipmentEquipped, setTrinketEquipmentEquipped, writeCharacters, type CharacterRecord, type PurchasedEquipment } from '../services/characters'
...
function purchaseGear(){
  ...
  const hasDedicatedTrinket=(c.equipment||[]).some(existing=>isTrinketGear(existing)&&existing.equipped===true&&existing.trinketSlot!=='shield')
  const equipped=kind?!((c.equipment||[]).some(existing=>protectiveGearKind(existing)===kind&&existing.equipped===true))&&!(kind==='shield'&&(c.equipment||[]).some(existing=>isTrinketGear(existing)&&existing.equipped===true&&existing.trinketSlot==='shield')):trinket?!hasDedicatedTrinket:undefined
  const purchased:PurchasedEquipment={name:item.name,costWp,costPaidWp:costWp,costSp:costWp/WP_PER_SP,costNp:costWp/WP_PER_NP,category:item.category,detail:gearDisplayDetail(item),effect:item.effect,choice:choice||undefined,quantity:1,statBonuses:item.statBonuses,equipped,trinketSlot:trinket&&equipped?'trinket':undefined}
...
function equipTrinket(index:number,slot:'trinket'|'shield'){
  ...
  const item=character.value?.equipment?.[index]
  message.value=item?.equipped?`${item.name} equipped in the ${item.trinketSlot==='shield'?'Shield':'Trinket'} slot.`:`${current.name} unequipped.`
}
