...
<template>
  <main class="page news-page"><AppHeader compact /><div class="page-title-block"><h1>News</h1><p>Current Brambleheart development and rules-update highlights.</p></div>
    <section class="news-promo-grid" aria-label="Brambleheart community and sharing"><button type="button" class="news-promo-card card-surface" @click="openNewsDiscord"><img :src="discordCommunityGraphic" alt="" aria-hidden="true" /><span><strong>Join the Brambleheart Discord</strong><small>Community discussion, play, feedback, and development updates.</small></span></button><button type="button" class="news-promo-card card-surface" @click="shareNewsSite"><img :src="shareBrambleheartGraphic" alt="" aria-hidden="true" /><span><strong>Share Brambleheart</strong><small>Send the companion to another player or copy the site link.</small></span></button></section>
    <section class="news-grid"><article class="news-card card-surface"><header><div><span class="eyebrow">LATEST SITE UPDATE</span><h2>{{ siteRelease.label }}</h2></div><RouterLink class="secondary-button news-action" to="/changelog">Site Changelog</RouterLink></header><ul><li v-for="item in siteRelease.items.slice(0,6)" :key="item">{{ item }}</li></ul></article><article v-if="currentRules" class="news-card card-surface"><header><div><span class="eyebrow">CURRENT RULE UPDATE</span><h2>Rules Update {{ currentRules.version }} · {{ currentRules.title }}</h2></div><RouterLink class="secondary-button news-action" to="/game-updates">Rule Updates</RouterLink></header><p>{{ currentRules.summary }}</p><ul><li v-for="item in currentRules.changes.slice(0,4)" :key="item">{{ item }}</li></ul></article></section>
  </main>
</template>
