...
const creatorFollowerComplete=ref(true)
...
const creatorFollowerLabel=computed(()=>creatorCountLoaded.value?(creatorFollowerCount.value===null?'Follower count unavailable':`${creatorFollowerCount.value.toLocaleString()}${creatorFollowerComplete.value?'':'+'} followers`):'Loading follower count…')
...
onMounted(async()=>{
  ...
  creatorFollowerCount.value=followers.count
  creatorFollowerComplete.value=followers.complete
  ...
  <main class="page news-page"><AppHeader compact /><div class="page-title-block"><h1>News</h1></div>
    <section class="news-promo-grid" aria-label="Brambleheart community and sharing">
      <a class="news-promo-card card-surface" :href="externalLinks.discord" target="_blank" rel="noopener noreferrer">
        ...
        <span class="news-promo-visual"><img class="discord-community-image" :src="discordCommunityGraphic" alt="" aria-hidden="true" /><small class="news-promo-count">{{ discordCountLabel }}</small></span>
        <small>Community discussion, play, feedback, and development updates.</small>
      </a>
      <button type="button" class="news-promo-card news-promo-card-share card-surface" @click="shareNewsSite">
        <strong>Share</strong>
        <span class="news-promo-visual"><img class="share-brambleheart-image" :src="shareBrambleheartGraphic" alt="" aria-hidden="true" /></span>
...
.news-promo-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin:10px 0 14px}.news-promo-card{display:grid;grid-template-rows:auto minmax(112px,1fr) auto;align-items:start;gap:8px;width:100%;min-width:0;min-height:210px;padding:12px 14px;border:1px solid var(--line-dark);background:var(--paper);color:var(--ink);text-align:center;text-decoration:none;cursor:pointer}.news-promo-card>strong{display:block;font-family:Georgia,'Times New Roman',serif;font-size:calc(20px + var(--font-offset));line-height:1.1}.news-promo-visual{display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:0;min-height:112px}.news-promo-card-share .news-promo-visual{justify-content:flex-start}.news-promo-visual img{display:block;width:auto;max-width:100%;object-fit:contain}.news-promo-visual .discord-community-image{height:144px}.news-promo-visual .share-brambleheart-image{height:144px;margin-top:10px}.news-promo-visual .creator-content-image{height:144px}.news-promo-card>small{color:var(--ink-soft);line-height:1.35}.news-promo-count{display:block;min-height:1.35em;margin-top:2px;color:color-mix(in srgb,var(--detail-trait-heritage) 72%,var(--ink-soft));font-weight:800;line-height:1.2}.news-promo-card:hover,.news-promo-card:focus-visible{border-color:var(--ui-active);background:var(--paper-2)}@media(max-width:720px){.news-promo-grid{grid-template-columns:1fr}.news-promo-card{min-height:180px}.news-promo-visual{min-height:96px}.news-promo-visual .discord-community-image,.news-promo-visual .creator-content-image{height:122px}.news-promo-visual .share-brambleheart-image{height:122px;margin-top:8px}}
</style>
