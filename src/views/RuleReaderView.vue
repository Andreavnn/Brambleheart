...
<template>
  <main class="page rule-reader-page">
    ...
    <template v-if="page">
      ...
      <template v-else-if="page.slug==='ability-targeting'">
        <section class="targeting-rule-page organized-rule-stack">
      ...
      <template v-else>
        ...
        <section v-if="page.slug==='spell-rules'" class="rule-cross-reference card-surface"><strong>Ability Targeting</strong><p>Combat Range, Direct, Line, Cone, Orb, Touch, and impassable collision are organized under The Battles as general Ability targeting rules.</p><RouterLink class="secondary-button compact-action" to="/rules/read/ability-targeting">Open Ability Targeting</RouterLink></section>
        <section v-if="sourceSections.length" class="rule-content-card card-surface">
          <article v-for="(entry,index) in sourceSections" :key="`${entry.document}-${entry.section.heading}-${index}`" class="rule-copy-card source-section-card source-rule-section" :class="{'example-source-card':isExampleHeading(entry.section.heading),'boxed-rule-entry':boxedRulePage()}">
