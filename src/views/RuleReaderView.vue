...
const monsterGroups=computed(()=>monsterCategories.map(category=>({category,monsters:externalMonsters.filter(monster=>monster.category===category.name)})))
  ...
  <main class="page rule-reader-page">
    ...
    <template v-if="page">
      ...
      <template v-else-if="page.slug==='monsters' || page.slug==='critters'">
        <section class="monster-index-stack"><article class="rule-feature-box monster-source-intro"><header><h2>{{ page.slug==='critters'?'Critters & Companions':'Monsters of Anthro Mundas' }}</h2></header><p>The Woodlands source organizes creatures into companions and monster families shaped by the wilds, magic, undeath, primordial forces, and the legacy of the Great Morphing.</p></article><template v-for="group in monsterGroups" :key="group.category.name"><section v-if="group.monsters.length && (page.slug==='monsters'?group.category.name!=='Companions':group.category.name==='Companions')" class="monster-category-box card-surface"><header><h2>{{ group.category.name }}</h2><p>{{ group.category.summary }}</p></header><div class="monster-link-grid"><RouterLink v-for="monster in group.monsters" :key="monster.name" :to="`/rules/read/${monsterSlug(monster.name)}`"><strong>{{ monster.name }}</strong><small v-if="monster.group">{{ monster.group }}</small></RouterLink></div></section></template><a class="secondary-button external-source-link" :href="WOODLANDS_MONSTERS_URL" target="_blank" rel="noopener noreferrer">Open Source Monster Reference</a></section>
      </template>
