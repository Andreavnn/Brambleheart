...
<style scoped>
.character-action-launch{display:grid!important;grid-template-columns:minmax(0,1fr)!important;align-items:stretch!important;gap:0!important}.combined-character-actions{width:100%;min-width:0;justify-content:flex-start}.character-filter-button{margin-left:auto}.character-filter-button.active{border-color:var(--accent);background:var(--accent-wash)}.character-filter-panel{box-sizing:border-box;display:grid;grid-template-columns:minmax(0,1fr) minmax(180px,.42fr) auto;gap:10px;align-items:end;width:100%;min-width:0;margin-top:12px;padding:12px 0 0;border-top:1px solid var(--line)}.character-filter-panel .field-label,.character-filter-panel .field-control{min-width:0;width:100%}.character-filter-panel>.secondary-button{min-width:76px}
