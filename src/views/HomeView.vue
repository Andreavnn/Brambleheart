        ...
        <article v-for="character in group.characters" :key="character.id" class="saved-list-card card-surface character-card" :class="{pinned:character.pinned,locked:character.locked,approved:characterStatus(character)==='approved',draft:characterStatus(character)==='incomplete','character-status-incomplete':characterStatus(character)==='incomplete','character-status-complete':characterStatus(character)!=='incomplete'}">
          <div class="character-card-topline">
            ...
            <div class="character-card-actions-visible" role="group" :aria-label="`${character.name} actions`">
              <button v-if="characterStatus(character)!=='approved'" class="secondary-button character-icon-action" type="button" :disabled="character.locked" aria-label="Edit character" :title="character.locked?'Unlock to Edit':'Edit'" @click="editCharacter(character.id)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4L19 9l-4-4L4 16v4Z"/><path d="m13.5 6.5 4 4"/></svg></button>
              <button v-else class="primary-button character-icon-action" type="button" :disabled="character.locked" aria-label="Level up character" :title="character.locked?'Unlock to Level Up':'Level Up'" @click="levelUpCharacter(character.id)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 19V5"/><path d="m6.5 10.5 5.5-5.5 5.5 5.5"/></svg></button>
