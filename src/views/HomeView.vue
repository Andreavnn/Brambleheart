...
import { derivedStats, equipmentControlBonus, equipmentGutsBonus, equipmentMagicRegenBonus, equipmentSpeedPenalty, magicResources, speciesMinimumSpeed } from '../rules/rulesEngine'
...
function levelUpCharacter(id:string){const character=characters.value.find(item=>item.id===id);if(!character||character.locked)return;void router.push(`/characters/${id}/level-up`)}
function editCharacter(id:string){const character=characters.value.find(item=>item.id===id);if(!character||character.locked)return;void router.push({path:'/characters/create',query:{edit:id}})}
function copyCharacter(id:string){const previous=snapshot();const source=characters.value.find(item=>item.id===id);if(!source)return;const now=new Date().toISOString();const complete=characterStatus(source)!=='incomplete';const copy:CharacterRecord={...source,id:crypto.randomUUID(),name:`${source.name} Copy`,creationComplete:complete,status:complete?'unapproved':'incomplete',draft:!complete,locked:false,pinned:false,exampleCharacter:false,createdAt:now,updatedAt:now};characters.value=[copy,...characters.value];persist(previous)}
function removeCharacter(id:string){const c=characters.value.find(item=>item.id===id);if(c&&(c.locked||(characterStatus(c)==='approved'&&!c.exampleCharacter))){alert(c.locked?'Unlock this character before deleting it.':'Remove approval before deleting this character.');return}if(!confirm('Delete this character from this device?'))return;const previous=snapshot();characters.value=characters.value.filter(item=>item.id!==id);persist(previous)}
...
function derived(c:CharacterRecord){return derivedStats(c.attributes,equipmentGutsBonus(c.equipment),equipmentControlBonus(c.equipment),equipmentSpeedPenalty(c.equipment,c.species==='Tordan'),speciesMinimumSpeed(c.species))}
  ...
  <main class="page home-page">
    ...
    <section class="list-launch card-surface character-action-launch">
      <div class="list-launch-actions centered-character-actions combined-character-actions">
        ...
        <button class="secondary-button character-filter-button" type="button" :class="{active:filterOpen}" aria-label="Filter characters" title="Filter" @click="filterOpen=!filterOpen"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 6h16M7 12h10M10 18h4"/></svg></button>
        ...
        <article v-for="character in group.characters" :key="character.id" class="saved-list-card card-surface character-card" :class="{pinned:character.pinned,locked:character.locked,approved:characterStatus(character)==='approved',draft:characterStatus(character)==='incomplete','character-status-incomplete':characterStatus(character)==='incomplete','character-status-complete':characterStatus(character)!=='incomplete'}">
          <div class="character-card-topline">
            ...
            <div class="character-card-actions-visible" role="group" :aria-label="`${character.name} actions`">
              <button v-if="characterStatus(character)!=='approved'||character.exampleCharacter" class="secondary-button character-icon-action" type="button" :disabled="character.locked" aria-label="Edit character" :title="character.locked?'Unlock to Edit':'Edit'" @click="editCharacter(character.id)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4L19 9l-4-4L4 16v4Z"/><path d="m13.5 6.5 4 4"/></svg></button>
              <button v-if="characterStatus(character)==='approved'" class="primary-button character-icon-action" type="button" :disabled="character.locked" aria-label="Level up character" :title="character.locked?'Unlock to Level Up':'Level Up'" @click="levelUpCharacter(character.id)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 19V5"/><path d="m6.5 10.5 5.5-5.5 5.5 5.5"/></svg></button>
              ...
              <button class="danger-button character-icon-action" type="button" :disabled="character.locked||(characterStatus(character)==='approved'&&!character.exampleCharacter)" aria-label="Delete character" title="Delete" @click="removeCharacter(character.id)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16"/><path d="M9 7V4h6v3"/><path d="m6 7 1 13h10l1-13"/><path d="M10 11v5M14 11v5"/></svg></button>
            </div>
          </div>
          <div v-if="openIds.has(character.id)" class="character-detail-panel">
            ...
            <div class="info-card character-derived"><strong>Derived Values</strong><p>Aim {{ derived(character).aim }} · Accuracy {{ derived(character).accuracy }} · Speed {{ derived(character).speed }} · Brawl {{ derived(character).brawl }} · Fury {{ derived(character).fury }} · Ward {{ derived(character).ward }} · Guts {{ derived(character).guts }} · Control {{ derived(character).control }} · Power {{ derived(character).power }} · Heart {{ derived(character).heart }} · Spirit {{ derived(character).spirit }} · Mana Pool {{ resources(character).manaPool }} · Magic Regen {{ resources(character).magicRegen }}.</p></div>
...
<style scoped>
.character-action-launch{display:grid;grid-template-columns:minmax(0,1fr);align-items:stretch;gap:0}.combined-character-actions{width:100%;min-width:0;justify-content:flex-start}.character-filter-button{margin-left:auto;width:36px;min-width:36px;height:36px;min-height:36px;padding:0;display:inline-flex;align-items:center;justify-content:center}.character-filter-button svg{width:17px;height:17px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round}.character-filter-button.active{border-color:var(--accent);background:var(--accent-wash)}.character-filter-panel{box-sizing:border-box;display:grid;grid-template-columns:minmax(0,1fr) minmax(180px,.42fr) auto;gap:10px;align-items:end;width:100%;min-width:0;margin-top:12px;padding:12px 0 0;border-top:1px solid var(--line)}.character-filter-panel .field-label,.character-filter-panel .field-control{min-width:0;width:100%}.character-filter-panel>.secondary-button{min-width:76px}
...
@media(max-width:680px){.combined-character-actions{display:grid;grid-template-columns:1fr 1fr}.combined-character-actions>*{width:100%;margin:0}.character-filter-panel,.transfer-choice-grid{grid-template-columns:1fr}.olddex-share-dialog{width:100%;max-height:92vh;overflow:auto;border-radius:14px 14px 0 0}.share-code-backdrop{align-items:flex-end}}
@media(max-width:620px){.combined-character-actions .character-filter-button{width:36px;min-width:36px;justify-self:end}}
</style>
