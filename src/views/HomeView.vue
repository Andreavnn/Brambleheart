...
function setLocked(id:string,locked:boolean){const previous=snapshot();const now=new Date().toISOString();characters.value=characters.value.map(item=>item.id===id?{...item,locked,updatedAt:now}:item);persist(previous)}
function toggleLock(id:string){const c=characters.value.find(item=>item.id===id);if(!c)return;setLocked(id,!c.locked)}
...
function exportCharacters(){if(!characters.value.length)return;downloadJson('brambleheart-characters.json',{format:'brambleheart-characters',version:'0.17',characters:characters.value})}
        ...
        <article v-for="character in group.characters" :key="character.id" class="saved-list-card card-surface character-card" :class="{pinned:character.pinned,locked:character.locked,draft:character.draft,'character-status-incomplete':character.draft,'character-status-complete':!character.draft}">
          <div class="character-card-topline">
            ...
            <div class="character-card-actions-visible">
              ...
              <button class="secondary-button compact-action character-lock-button" type="button" :class="{active:character.locked}" @click="toggleLock(character.id)">{{ character.draft?(character.locked?'Unlock':'Lock'):(character.locked?'Remove Approval':'Approve') }}</button>
