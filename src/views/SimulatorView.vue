...
function talentAbilityTypes(name:string){return Array.from(new Set(talentKeywords(name).map(canonicalAbilityType).filter(value=>ABILITY_TYPE_KEYWORDS.has(value))))}
function talentAbilityClasses(name:string){return talentAbilityTypes(name).map(abilityTypeClass)}
function spellAbilityTypes(name:string){return Array.from(new Set(activeAbilityKeywords(spellDetail(name)?.keywords).map(canonicalAbilityType).filter(value=>ABILITY_TYPE_KEYWORDS.has(value))))}
