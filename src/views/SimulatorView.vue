...
const primaryTrinket=computed(()=>primaryTrinketGear(selectedCharacter.value?.equipment))
const secondaryTrinket=computed(()=>secondaryTrinketGear(selectedCharacter.value?.equipment))
  ...
  <main class="page simulator-page rhythm-engine-page">
    ...
    <section v-else class="character-sheet-view">
      <section v-if="selectedCharacter" class="digital-character-sheet card-surface review-character-sheet">
        ...
        <details class="review-section review-collapsible review-armor-section" open><summary><h2>Armor &amp; Shield</h2><span>{{ armor.length + (secondaryTrinket ? 1 : 0) }} / 2 protective slots</span></summary><div class="review-collapsible-body"><div class="equipment-slot-sheet"><div class="equipment-slot-header armor-slot-grid"><span>Armor / Shield</span><span>Guts</span><span>Mana Syphon</span><span>Stealth</span><span>Might</span><span>Wt.</span></div><div v-for="(slot,index) in armorSlots" :key="`armor-slot-${index}`" class="equipment-slot-row armor-slot-grid"><strong>{{ slot.name }}</strong><span>{{ slot.guts }}</span><span>{{ slot.mana }}</span><span>{{ slot.stealth }}</span><span>{{ slot.might }}</span><span>{{ slot.weight }}</span></div></div><div class="trinket-slot-sheet"><h3>Trinkets</h3><div class="trinket-slot-grid"><article class="trinket-slot-card"><span>Trinket Slot</span><strong>{{ primaryTrinket?.name || '—' }}</strong><small>{{ primaryTrinket?.effect || 'Empty Trinket slot.' }}</small></article><article class="trinket-slot-card secondary"><span>Shield / Trinket Slot</span><strong>{{ secondaryTrinket?.name || '—' }}</strong><small>{{ secondaryTrinket?.effect || 'A second Trinket may use this slot in place of a Shield.' }}</small></article></div></div></div></details>
