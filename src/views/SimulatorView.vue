...
const effectiveRhythmConditions=computed(()=>Number(rhythmConditions.value||0)+(rollType.value==='Compelled'?gearRenewHeartBonus.value:0))
...
const otherGear=computed(()=>selectedCharacter.value?.equipment?.filter(item=>!['Weapon','Armor & Shield','Trinket'].includes(item.category||''))||[])
...
const dedicatedTrinket=computed(()=>dedicatedTrinketGear(selectedCharacter.value?.equipment))
const overflowTrinket=computed(()=>overflowTrinketGear(selectedCharacter.value?.equipment))
...
const characterSpells=computed(()=>{const c=selectedCharacter.value;if(!c||magicLevel.value<1)return[];return Array.from(new Set([signatureSpell.value,...(c.spells||[]),...(c.invocationSpells||[]),...(c.invocationSpell?[c.invocationSpell]:[])].filter(Boolean) as string[]))})
  ...
  <main class="page simulator-page rhythm-engine-page">
    ...
    <section v-else class="character-sheet-view">
      <section v-if="selectedCharacter" class="digital-character-sheet card-surface review-character-sheet">
        ...
        <details class="review-section review-collapsible review-armor-section" open><summary><h2>Armor &amp; Shield</h2><span>{{ armor.length + (overflowTrinket ? 1 : 0) }} / 2 protective slots</span></summary><div class="review-collapsible-body"><div class="equipment-slot-sheet"><div class="equipment-slot-header armor-slot-grid"><span>Armor / Shield</span><span>Guts</span><span>Mana Syphon</span><span>Stealth</span><span>Might</span><span>Wt.</span></div><div v-for="(slot,index) in armorSlots" :key="`armor-slot-${index}`" class="equipment-slot-row armor-slot-grid"><strong>{{ slot.name }}</strong><span>{{ slot.guts }}</span><span>{{ slot.mana }}</span><span>{{ slot.stealth }}</span><span>{{ slot.might }}</span><span>{{ slot.weight }}</span></div></div><div class="trinket-slot-sheet"><h3>Trinket</h3><div class="trinket-slot-row"><span>Trinket Slot</span><strong>{{ dedicatedTrinket?.name || '—' }}</strong><small>{{ dedicatedTrinket?.effect || 'One equipped Trinket can use this slot.' }}</small></div><div v-if="overflowTrinket" class="trinket-slot-row overflow"><span>Armor &amp; Shield Slot</span><strong>{{ overflowTrinket.name }}</strong><small>{{ overflowTrinket.effect }}</small></div></div></div></details>
