  ...
  <main class="page simulator-page rhythm-engine-page">
    ...
    <section v-else class="character-sheet-view">
      <section v-if="selectedCharacter" class="digital-character-sheet card-surface review-character-sheet">
        ...
        <details class="review-section review-collapsible character-sheet-attributes" open><summary><h2>Attributes</h2><span>Ranks &amp; derived stats</span></summary><div class="review-collapsible-body"><div class="review-attribute-cards single-row-attributes"><article v-for="attribute in attributes" :key="attribute.id"><strong>{{ attribute.name }}</strong><span>Rank {{ selectedCharacter.attributes[attribute.id] }}</span><small>Modifier +{{ attributeModifier(selectedCharacter.attributes[attribute.id]) }}</small></article></div><div v-if="secondaryStats" class="review-secondary-strip"><article><span>Speed</span><strong>{{ secondaryStats.speed }}</strong></article><article><span>Aim</span><strong>{{ secondaryStats.aim }}</strong></article><article><span>Mettle</span><strong>{{ secondaryStats.mettle }}</strong></article><article><span>Ward</span><strong>{{ secondaryStats.ward }}</strong></article><article><span>Control</span><strong>{{ secondaryStats.control }}</strong></article><article><span>Power</span><strong>{{ secondaryStats.power }}</strong></article><article><span>Guts</span><strong>{{ reviewGuts }}</strong><small v-if="gearGutsBonus">{{ secondaryStats.guts }} + {{ gearGutsBonus }} gear</small></article><article class="review-mana-stat"><span>Mana</span><small>Max / Round</small><strong>{{ maxMana }} / +{{ manaPerRound }}</strong></article></div></div></details>
