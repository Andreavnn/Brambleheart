...
function newEncounter(){
  ...
  encounters.value=[record,...encounters.value];activeEncounterId.value=record.id;encounterName.value='';newEncounterCharacterId.value='';resetTurnAbilities();persistEncounters()
}
function selectEncounter(id:string){activeEncounterId.value=id;resetTurnAbilities();combatResult.value=null}
...
function changeRound(delta:number){const item=activeEncounter.value;if(!item)return;const before=item.round;item.round=Math.max(1,item.round+delta);if(item.round>before){item.mana+=characterManaRegen.value;addEvent(`Round ${item.round}: +${characterManaRegen.value} Mana regenerated.`)}else{item.updatedAt=stamp();persistEncounters()}resetTurnAbilities()}
...
function reopenEncounter(id:string){const item=encounters.value.find(e=>e.id===id);if(!item)return;item.status='ongoing';item.updatedAt=stamp();activeEncounterId.value=id;resetTurnAbilities();persistEncounters()}
...
function resetTurnAbilities(){abilityTypes.forEach(type=>abilitySpent[type]=false)}
function toggleAbility(type:(typeof abilityTypes)[number]){abilitySpent[type]=!abilitySpent[type]}
const abilityStatus=computed(()=>abilityRemaining.value.length===0?'No Root ability types remain this turn. Passive Abilities may still apply when their trigger and restrictions are met.':abilityRemaining.value.length===4?'Turn ready: all four Root ability types are available.':`${abilityRemaining.value.length} Root ability type${abilityRemaining.value.length===1?'':'s'} remaining this turn.`)
