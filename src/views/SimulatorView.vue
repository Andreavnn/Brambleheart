...
import { armorProfile, derivedStats, equipmentControlBonus, equipmentGutsBonus, magicResources, normalizeSkillName, rankModifier, rhythmResult, structuredRule, visibleRuleFields, weaponProfile } from '../rules/rulesEngine'
import { formatGearCostNp, gearCostNp } from '../rules/threadpieces'
...
const allCharacterStats=computed(()=>{
  ...
  const secondary=derivedStats(c.attributes,equipmentGutsBonus(c.equipment),equipmentControlBonus(c.equipment))
  return[
...
const rollSummary=computed(()=>{const style=rhythmMode.value==='normal'?'Normal':rhythmMode.value==='edged'?'Edged':'Weighted';const stat=selectedStat.value?.label||statFieldLabel.value;const parts=[`${stat} (${effectiveRhythmStat.value})`];if(rollUsesSkill.value&&rhythmSkill.value)parts.push(`${rhythmSkill.value} (${Number(rhythmSkillBonus.value||0)})`);const conditions=Number(rhythmConditions.value||0);if(conditions!==0)parts.push(`Conditions (${conditions})`);return`${style} ${rollType.value} Roll: ${parts.join(' + ')} = ${rhythmCombinedStat.value+conditions}`})
...
const secondaryStats=computed(()=>selectedCharacter.value?derivedStats(selectedCharacter.value.attributes,gearGutsBonus.value,gearControlBonus.value):null)
const reviewGuts=computed(()=>secondaryStats.value?.guts||0)
...
const resources=computed(()=>selectedCharacter.value?magicResources(selectedCharacter.value.attributes,magicLevel.value):{manaPool:0,manaPerRound:0})
const manaPerRound=computed(()=>resources.value.manaPerRound)
...
<template>
  <main class="page simulator-page rhythm-engine-page">
    ...
    <div class="segment-tabs simulator-tabs rhythm-sheet-tabs"><button type="button" class="segment-tab" :class="{active:sheet==='character'}" @click="sheet='character'">Character Sheet</button><button type="button" class="segment-tab" :class="{active:sheet==='attribute'}" @click="sheet='attribute'">Dice Roller</button></div>
    ...
    <section v-else class="character-sheet-view">
      <section v-if="selectedCharacter" class="digital-character-sheet card-surface review-character-sheet">
        ...
        <details class="review-section review-collapsible character-sheet-attributes" open><summary><h2>Attributes</h2><span>Ranks &amp; secondary stats</span></summary><div class="review-collapsible-body"><div v-if="secondaryStats" class="rhythm-attribute-stat-groups">
          <article class="rhythm-attribute-stat-group"><header><strong>Agility</strong><small>Rank {{ selectedCharacter.attributes.agility }} · Modifier +{{ attributeModifier(selectedCharacter.attributes.agility) }}</small></header><div><span>Aim</span><strong>{{ secondaryStats.aim }}</strong><small>Agility modifier</small></div><div><span>Accuracy</span><strong>{{ secondaryStats.accuracy }}</strong><small>Agility Rank · ranged damage</small></div></article>
          <article class="rhythm-attribute-stat-group"><header><strong>Might</strong><small>Rank {{ selectedCharacter.attributes.might }} · Modifier +{{ attributeModifier(selectedCharacter.attributes.might) }}</small></header><div><span>Brawl</span><strong>{{ secondaryStats.brawl }}</strong><small>Might modifier</small></div><div><span>Fury</span><strong>{{ secondaryStats.fury }}</strong><small>Might Rank · melee damage</small></div></article>
          <article class="rhythm-attribute-stat-group"><header><strong>Hide</strong><small>Rank {{ selectedCharacter.attributes.hide }} · Modifier +{{ attributeModifier(selectedCharacter.attributes.hide) }}</small></header><div><span>Ward</span><strong>{{ secondaryStats.ward }}</strong><small>Hide modifier</small></div><div><span>Guts</span><strong>{{ reviewGuts }}</strong><small>{{ secondaryStats.guts-gearGutsBonus }} Hide Rank<span v-if="gearGutsBonus"> + {{ gearGutsBonus }} gear</span></small></div></article>
          <article class="rhythm-attribute-stat-group"><header><strong>Lore</strong><small>Rank {{ selectedCharacter.attributes.lore }} · Modifier +{{ attributeModifier(selectedCharacter.attributes.lore) }}</small></header><div><span>Control</span><strong>{{ secondaryStats.control }}</strong><small>{{ secondaryStats.control-gearControlBonus }} Lore modifier<span v-if="gearControlBonus"> + {{ gearControlBonus }} gear</span></small></div><div><span>Power</span><strong>{{ secondaryStats.power }}</strong><small>Lore Rank · magical damage</small></div></article>
          <article class="rhythm-attribute-stat-group"><header><strong>Bravery</strong><small>Rank {{ selectedCharacter.attributes.bravery }} · Modifier +{{ attributeModifier(selectedCharacter.attributes.bravery) }}</small></header><div><span>Spirit</span><strong>{{ secondaryStats.spirit }}</strong><small>Bravery Rank · Mana restoration</small></div><div><span>Heart</span><strong>{{ secondaryStats.heart }}</strong><small>Bravery modifier · Mana Pool</small></div></article>
        </div><div v-if="secondaryStats" class="rhythm-resource-stat-row"><article><span>Mana Pool</span><strong>{{ maxMana }}</strong><small>Magic Level {{ magicLevel }} + Heart {{ secondaryStats.heart }} · restores +{{ manaPerRound }}/round</small></article><article><span>Speed</span><strong>{{ secondaryStats.speed }}</strong><small>2 + Agility modifier (+{{ attributeModifier(selectedCharacter.attributes.agility) }})</small></article></div></div></details>
...
<style scoped>
.custom-content-badge{display:inline-flex;align-items:center;margin-left:5px;padding:2px 5px;border:1px solid #7b4f8d;border-radius:999px;background:#efe5f4;color:#60336f;font-size:8px;font-weight:900;letter-spacing:.06em;vertical-align:middle}.rhythm-sheet-tabs{margin-bottom:8px}.character-selection-panel{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:end;gap:12px;margin-bottom:12px;padding:10px 12px}.character-selection-summary{display:grid;gap:2px;text-align:right}.character-selection-summary span{color:var(--ink-soft);font-size:calc(9px + var(--font-offset))}.fortune-tip{display:grid;gap:4px;margin:12px 0;padding:10px 12px;border:1px solid var(--line);border-left:5px solid var(--accent);border-radius:9px;background:var(--paper-2)}.fortune-tip strong{font-family:Georgia,'Times New Roman',serif}.fortune-tip span{color:var(--ink-soft);line-height:1.5}.field-off{opacity:.68}.disabled-readout{display:flex;align-items:center;background:var(--paper-2);color:var(--ink-soft);cursor:not-allowed}.review-character-sheet{display:grid;gap:10px;padding:12px;min-width:0;max-width:100%;overflow:hidden}.review-character-sheet>*{min-width:0;max-width:100%}.review-character-sheet>.review-section{width:100%;box-sizing:border-box;min-width:0}.character-detail-body{display:grid;gap:10px}.character-detail-body .review-summary-grid{margin-top:0}.equipment-slot-sheet{display:block!important;max-width:100%;overflow-x:auto}.weapon-slot-grid{min-width:620px}.armor-slot-grid{min-width:720px}.rhythm-dependent-grid,.rhythm-target-row{row-gap:12px!important}.rhythm-target-row{margin-top:12px!important}.roll-formula-summary{margin-top:12px!important}.rhythm-tool-card .tool-explainer{margin-bottom:10px}.rhythm-attribute-stat-groups{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px}.rhythm-attribute-stat-group{min-width:0;border:1px solid var(--line);border-radius:9px;overflow:hidden;background:var(--paper)}.rhythm-attribute-stat-group>header{display:grid;gap:2px;padding:8px;background:var(--paper-2);border-bottom:1px solid var(--line)}.rhythm-attribute-stat-group>header strong{font-family:Georgia,'Times New Roman',serif}.rhythm-attribute-stat-group>header small{color:var(--ink-soft);line-height:1.3}.rhythm-attribute-stat-group>div{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:2px 6px;align-items:baseline;padding:8px;border-top:1px solid var(--line)}.rhythm-attribute-stat-group>header+div{border-top:0}.rhythm-attribute-stat-group>div span{font-weight:800}.rhythm-attribute-stat-group>div strong{font-size:calc(18px + var(--font-offset));color:var(--accent-dark)}.rhythm-attribute-stat-group>div small{grid-column:1/-1;color:var(--ink-soft);font-size:calc(9px + var(--font-offset));line-height:1.3}.rhythm-resource-stat-row{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:8px}.rhythm-resource-stat-row article{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:2px 8px;align-items:baseline;padding:9px 11px;border:1px solid var(--line);border-radius:9px;background:var(--paper-2)}.rhythm-resource-stat-row span{font-weight:850}.rhythm-resource-stat-row strong{font-size:calc(20px + var(--font-offset));color:var(--accent-dark)}.rhythm-resource-stat-row small{grid-column:1/-1;color:var(--ink-soft)}@media(max-width:760px){.character-selection-panel{grid-template-columns:1fr}.character-selection-summary{text-align:left}.rhythm-attribute-stat-groups{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:520px){.rhythm-attribute-stat-groups,.rhythm-resource-stat-row{grid-template-columns:1fr}}
</style>
