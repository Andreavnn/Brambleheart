...
function procResultsFor(source:AbilityManagerEntry,pool:AbilityManagerEntry[]){return pool.flatMap(entry=>{const match=findAbilityProcMatch(source,entry,generalAbilityLibrary.value);return match?[{entry,match}]:[]}).sort((a,b)=>b.match.score-a.match.score||a.entry.source.localeCompare(b.entry.source)||a.entry.name.localeCompare(b.entry.name))}
function buildCompleteChain(root:AbilityManagerEntry,pool:AbilityManagerEntry[]):AbilityChainRow[]{
  ...
  while(queue.length){const current=queue.shift()!;for(const result of procResultsFor(current.entry,pool)){if(seen.has(result.entry.id))continue;seen.add(result.entry.id);rows.push({entry:result.entry,match:result.match,depth:current.depth+1,parentName:current.entry.name});queue.push({entry:result.entry,depth:current.depth+1})}}
  return rows
}
...
const characterChainGroups=computed<AbilityChainGroup[]>(()=>{const pool=characterAbilityPool.value;if(!abilityManagerCharacter.value||!pool.length)return[];return pool.map(root=>({root,rows:buildCompleteChain(root,pool)})).filter(group=>group.rows.length>1).sort((a,b)=>b.rows.length-a.rows.length||a.root.source.localeCompare(b.root.source)||a.root.name.localeCompare(b.root.name))})
...
<template>
  <main class="page simulator-page rhythm-engine-page">
    ...
    <section v-else-if="sheet==='ability'" class="tool-panel card-surface ability-manager-card">
      <div class="tool-heading"><div><p class="eyebrow">ABILITY MANAGER</p><h2>Ability Proc &amp; Chain Simulator</h2></div><button v-if="abilityManagerMode" type="button" class="secondary-button compact-action" @click="clearAbilityManager">Start Over</button></div>
      ...
      <template v-else-if="abilityManagerMode==='ability'">
        ...
        <div v-if="selectedManagerAbility" class="ability-manager-summary"><article class="ability-manager-focus"><small>Starting Ability</small><strong>{{ selectedManagerAbility.name }}</strong><span>{{ selectedManagerAbility.source }}</span><div class="keyword-pill-row"><span v-for="keyword in selectedManagerAbility.keywords" :key="keyword" :class="talentAbilityKeywords.has(canonicalAbilityType(keyword))?['ability-cost-pill',`type-${canonicalAbilityType(keyword).toLowerCase().replace(/[^a-z0-9]+/g,'-')}`]:'keyword-pill'">{{ canonicalAbilityType(keyword) }}</span></div></article><article class="ability-manager-focus"><small>Complete Chain</small><strong>{{ Math.max(0,selectedAbilityChain.length-1) }} downstream procs</strong><span>The chain follows each newly triggered ability and continues until no additional unused ability in the rules library can proc.</span></article></div>
