...
function spellMana(name:string){return Number(spellDetail(name)?.manaCost??0)}
const loreCharacterSpells=computed(()=>characterSpells.value.filter(name=>!selectedCharacter.value?.invocationSpells?.includes(name)).sort((a,b)=>spellMana(a)-spellMana(b)||a.localeCompare(b)))
const invocationCharacterSpells=computed(()=>characterSpells.value.filter(name=>selectedCharacter.value?.invocationSpells?.includes(name)).sort((a,b)=>spellMana(a)-spellMana(b)||a.localeCompare(b)))
</script>
  ...
  <main class="page simulator-page rhythm-engine-page">
    ...
    <section v-else class="character-sheet-view">
      <section v-if="selectedCharacter" class="digital-character-sheet card-surface">
        ...
        <section v-if="secondaryStats" class="sheet-section"><h3>Secondary Stats</h3><div class="sheet-stat-grid secondary"><article v-for="(value,key) in secondaryStats" :key="key"><span>{{ String(key)[0].toUpperCase()+String(key).slice(1) }}</span><strong>{{ value }}</strong></article><article class="sheet-mana-stat"><span>Mana</span><small>Max / Round</small><strong>{{ maxMana }} / +{{ manaPerRound }}</strong></article></div></section>
        <div class="sheet-two-column"><section class="sheet-section"><h3>Skills</h3><div class="sheet-list-grid"><article v-for="skill in selectedSkills" :key="skill.name"><strong>{{ skill.name }}</strong><span>Rank {{ skill.rank }}</span><small>Modifier +{{ skillModifier(skill.rank) }}</small></article></div></section><section class="sheet-section"><h3>Talents</h3><div class="sheet-rule-stack"><article v-for="talent in selectedCharacter.talents||[]" :key="talent" class="review-feature-box talent full-rule-entry"><header><strong>{{ talent }} <small v-if="isCustomTalent(talent)" class="custom-content-badge">CUSTOM</small></strong></header><p><em>{{ talentText(talent) }}</em></p></article><p v-if="!selectedCharacter.talents?.length" class="muted">No Talents recorded.</p></div></section></div>
        ...
        <section v-if="selectedCharacter.path==='magic'" class="sheet-section"><h3>Magic &amp; Spells</h3><div class="sheet-lore-banner"><div><strong>Lore of {{ selectedCharacter.loreAttunement||'—' }}</strong><small>{{ loreDescriptions[selectedCharacter.loreAttunement||'']||'' }}</small></div><span>Mana {{ maxMana }} Max · +{{ manaPerRound }}/round</span></div><h4>Lore Spells</h4><div class="sheet-rule-grid single-column"><article v-for="spell in loreCharacterSpells" :key="spell" class="review-feature-box spell full-rule-entry"><header><strong>{{ spell }} <small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></strong><span>{{ spellMana(spell) }} Mana</span></header><p><em>{{ spellDetail(spell)?.flavor }}</em></p><small>{{ spellDetail(spell)?.rules }}</small></article></div><h4>Invocation Spells</h4><div class="sheet-rule-grid single-column"><article v-for="spell in invocationCharacterSpells" :key="spell" class="review-feature-box spell full-rule-entry"><header><strong>{{ spell }} <small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></strong><span>{{ spellMana(spell) }} Mana</span></header><p><em>{{ spellDetail(spell)?.flavor }}</em></p><small>{{ spellDetail(spell)?.rules }}</small></article><p v-if="!invocationCharacterSpells.length" class="muted">No Invocation Spells recorded.</p></div></section>
