...
function targetLabel(){
  if(targetMode.value==='active')return activeTarget.value===null?'Active target':'Active target'
  if(targetMode.value==='passive')return passiveTargets.find(([,value])=>value===Number(passiveTarget.value))?.[0]||'Passive target'
  return 'No target'
}
function rollRhythm(){
  const result=rhythmResult(dice(rhythmMode.value==='normal'?3:4),rhythmMode.value,rhythmStat.value,rhythmConditions.value)
  const kept=result.kept||result.rolled
  const entry:RhythmHistoryEntry={
    id:makeId(),createdAt:stamp(),mode:rhythmMode.value,target:currentTarget.value,targetLabel:targetLabel(),result,
...
function rollOutcome(entry:RhythmHistoryEntry){
  if(entry.target===null)return''
  return entry.result.total>=entry.target?'SUCCESS':'FAILURE'
}
...
<template>
  <main class="page simulator-page">
    ...
    <section v-if="tab==='rhythm'" class="tool-panel card-surface rhythm-tool-card">
      ...
      <p class="tool-explainer">Most checks use three ten-sided dice plus the required stat and any conditions. Edged rolls add a fourth die and remove the lowest; weighted rolls remove the highest. Fortune and Misfortune results are shown from the natural kept dice.</p>
      ...
      <div v-if="rhythmHistory.length" class="rhythm-history-list">
        <article v-for="(entry,index) in rhythmHistory" :key="entry.id" class="result-panel rhythm-history-entry" :class="{latest:index===0}">
          ...
          <small>{{ entry.result.natural }} natural + {{ entry.result.stat }} stat {{ entry.result.conditions>=0?'+':'−' }} {{ Math.abs(entry.result.conditions) }} conditions<template v-if="entry.result.dropped!==null"> · dropped {{ entry.result.dropped }}</template></small>
        </article>
      </div>
