...
import IndependentCardColumns from '../components/IndependentCardColumns.vue'
...
import { cultureSkillGrants, speciesImagePaths, startingPathOptions } from '../data/creationRules'
...
import { TALENT_CATEGORIES, canonicalTalentName, classifyTalent, talentFeaturePillKeywords, talentManaCost, talentNameMatches, talentToneClasses } from '../data/talentCategories'
...
import { ADVENTURE_KIT_ITEMS, adventureKitEquipment } from '../data/equipment'
import { deeds } from '../data/deeds'
import { XP_GUIDANCE } from '../rules/advancementRules'
import { characterThreatLevel } from '../rules/threatLevel'
import { equipmentCardFields, equipmentToneClass, mergeEquipmentPresentation } from '../rules/equipmentPresentation'
...
const woodlandSpecies=computed(()=>[...species].sort())
const availableCustomSpecies=computed(()=>form.allowCustomData?Array.from(new Set(customSpeciesItems.value.map(item=>item.name))).sort():[])
...
function equipmentQuantity(item:{quantity?:number}){return Math.max(1,Math.floor(Number(item.quantity)||1))}
...
function formatGearCost(item:{name:string;costWp?:number;costSp?:number;costNp?:number}){return formatThreadpieceWp(gearCostWp(item))}
...
const currentDeedExample=computed(()=>{const keys=new Set(currentSpark.value?.keywords||[]);return deeds.find(deed=>deed.keywords.some(keyword=>keys.has(keyword)))||deeds[0]})
...
const reviewControl=computed(()=>derivedStats(effectiveAttributes.value,gearGutsBonus.value,gearControlBonus.value,gearSpeedPenalty.value,minimumSpeed.value).control)
...
function cultureFooterKeywords(values:string[]|undefined,speciesName:string,grantsSkill=false){return traitFooterKeywords(values,'Cultural',speciesName,grantsSkill)}
...
const reviewSkillEntries=computed(()=>Object.entries(skillRanks.value).map(([name,rank])=>({name,rank:Number(rank)})))
const currentThreatLevel=computed(()=>characterThreatLevel({attributeRanks:attributes.map(attribute=>Number(effectiveAttributes.value[attribute.id]||1)),skillRanks:Object.values(skillRanks.value),talentCount:form.talents.filter(Boolean).length,magicLevel:magicLevel.value}))
...
function gearDisplayDetail(item:typeof gearShopItems[number]){
  ...
  return item.detail&&item.detail!=='—'?`Weight ${item.detail}`:(gearEffect(item)?'':'No weight or numeric profile is listed in the current source.')
}
...
function addEquipment(item:typeof gearShopItems[number]){
  ...
  const costWp=gearCostWp(item),choice=gearChoice.value[item.name]||''
  ...
  const kind=protectiveGearKind(item),trinket=isTrinketGear(item)
  const equipped=kind?!form.equipment.some(candidate=>protectiveGearKind(candidate)===kind&&candidate.equipped===true):trinket?true:undefined
...
function purchaseFromSharedShop(payload:{item:typeof gearShopItems[number];choice:string}){gearChoice.value[payload.item.name]=payload.choice;addEquipment(payload.item)}
...
function toggleTrinketEquipped(index:number){
  const current=form.equipment[index];if(!current)return
  form.equipment=setTrinketEquipmentEquipped(form.equipment,index,current.equipped!==true)
}
...
const reviewTrinkets=computed(()=>equippedTrinketGear(form.equipment))
const reviewEquipmentItems=computed(()=>[...adventureKitEquipment(form.adventureKit),...form.equipment.filter(item=>item.category!=='Weapon'&&item.category!=='Armor & Shield'&&!isTrinketGear(item))])
const reviewEquipmentCards=computed(()=>reviewEquipmentItems.value.map(item=>mergeEquipmentPresentation(item,gearShopItems)))
...
function buildRecord(saveAsDraft:boolean):CharacterRecord{
  ...
  const firstCompletionWalkingMoney=!saveAsDraft&&originalStatus.value==='incomplete'&&wealthRemaining.value===0?10:0
  const wealthWp=preserveCampaignEconomy?previous!.wealthWp:(firstCompletionWalkingMoney||wealthRemaining.value)
...
onMounted(async()=>{
  ...
  if(!record)return
  if(record.locked){alert('Unlock this character before editing it.');void router.replace('/characters');return}
  ...
  <div class="app-page creator-page">
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
        <template v-if="stepId==='species'">
          ...
          <div class="species-choice-layout">
            <div class="species-choice-copy">
              <label class="field-label">Species<select v-model="form.species" class="field-control"><option value="">Select Species</option><optgroup label="Woodlands"><option v-for="item in woodlandSpecies" :key="`woodlands-${item}`" :value="item">{{ item }}</option></optgroup><optgroup v-if="availableCustomSpecies.length" label="Custom"><option v-for="item in availableCustomSpecies" :key="`custom-${item}`" :value="item">{{ item }}</option></optgroup></select></label>
              ...
              <template v-if="selectedSpeciesData">
                <details class="creation-info-panel species-lore-panel" :open="menusExpanded"><summary>{{ selectedSpeciesData.name }} Lore <span v-if="selectedSpeciesData.custom" class="custom-content-badge">CUSTOM</span></summary><div class="creation-info-body"><p class="species-quote">“{{ selectedSpeciesData.quote }}”</p><p>{{ selectedSpeciesData.lore }}</p><div class="keyword-pill-row"><span class="keyword-pill">{{ selectedSpeciesData.theme }}</span><span class="keyword-pill">{{ selectedSpeciesData.language.split(',')[0] }}</span></div></div></details>
                <details class="creation-info-panel trait-panel species-trait-panel"><summary>Heritage Traits</summary><div class="creation-info-body trait-stack"><TraitCard v-for="trait in selectedSpeciesTraits" :key="trait.name" :title="trait.name" :subtitle="`${form.species} · Heritage`" :flavor="structuredRule(trait.text).intro" :fields="visibleRuleFields(trait.text)" :keywords="traitFooterKeywords(trait.keywords,'Heritage',form.species)" :tone-class="['species-trait-card','heritage-trait-card']" :cost="manaCostFromText(trait.text)!==null?`${manaCostFromText(trait.text)} Mana`:''" :custom="'custom' in trait&&Boolean(trait.custom)" /></div></details>
                <details class="creation-info-panel trait-panel culture-trait-panel"><summary>Culture Traits</summary><div class="creation-info-body"><p>Your Species begins with these two Culture Traits. In Step 2 you may keep them or exchange them for Culture Traits from other peoples.</p><div class="trait-stack"><TraitCard v-for="trait in selectedSpeciesData.cultureTraits" :key="trait.name" :title="trait.name" :subtitle="`${form.species} · Cultural`" :flavor="structuredRule(trait.text).intro" :fields="visibleRuleFields(trait.text)" :keywords="cultureFooterKeywords(trait.keywords,form.species,Boolean(cultureGrant(cultureId(form.species,trait.name))))" tone-class="culture-trait-card" :cost="manaCostFromText(trait.text)!==null?`${manaCostFromText(trait.text)} Mana`:''" :custom="Boolean(selectedSpeciesData.custom)" /></div></div></details>
              </template>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="stepId==='spark'">
          ...
          <details class="creation-info-panel help-panel" :open="menusExpanded"><summary>What is a Spark &amp; Deed?</summary><div class="creation-info-body"><p>A Spark is the personality archetype that describes what tends to move your hero into action. Each Spark has two keywords that describe its nature.</p><p><strong>Deeds</strong> are meaningful acts completed during play — moments of courage, wit, compassion, discovery, or conviction that shape who your hero becomes. A completed Deed grants Experience based on its challenge and impact. If the Deed shares a keyword with your Spark, it grants <strong>+{{ XP_GUIDANCE.sparkAlignmentBonus }} bonus XP</strong> in addition to the normal reward, limited to once per session.</p><RouterLink to="/rules/read/sparks-deeds" class="inline-rule-link">Read Sparks &amp; Deeds →</RouterLink></div></details><article class="deed-example-card"><span class="eyebrow">EXAMPLE DEED FOR {{ form.spark.toUpperCase() }}</span><h2>{{ currentDeedExample.name }}</h2><p>{{ currentDeedExample.objective }}</p><div class="deed-example-meta"><span class="deed-bonus-reward"><strong>Bonus Reward</strong> +{{ currentDeedExample.rewardXp }} XP</span><div class="keyword-pill-row"><span v-for="keyword in currentDeedExample.keywords" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></div></article>
          ...
          <article v-if="form.spark" class="choice-summary spark-description-card compact-spark-preview detail-tone-spark"><h2>{{ form.spark }}</h2><p>{{ currentSpark?.description }}</p><div class="keyword-pill-row"><span v-for="keyword in activeAbilityKeywords(currentSpark?.keywords||[])" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article>
        </template>
        ...
        <template v-else-if="stepId==='attributes'">
          ...
          <details class="creation-info-panel help-panel" :open="menusExpanded"><summary>What do Attributes do?</summary><div class="creation-info-body"><p>Attributes are the five pillars of your character’s natural capability. Each begins at Rank 1. Spend 5 points to raise them, with a maximum Rank of 3 during character creation. Attribute Rank determines its modifier and helps form secondary stats used by other rules.</p><RouterLink to="/rules/read/attributes-skills" class="inline-rule-link">Read Attributes &amp; Skills →</RouterLink></div></details>
          <div class="subsection-title"><strong>Spend 5 Points</strong><span>{{ remaining }} points remaining</span></div>
          <div class="attribute-editor attribute-sheet-editor"><article v-for="attribute in attributes" :key="attribute.id" class="attribute-sheet-row" :class="`attribute-${attribute.id}`"><div class="attribute-sheet-copy"><div class="attribute-sheet-title"><strong>{{ attribute.name }}</strong></div><small>{{ attribute.description }}</small><div class="attribute-sheet-secondary"><span v-for="secondary in secondaryStats[attribute.id]" :key="secondary.name"><RouterLink v-if="secondary.slug" :to="`/rules/read/${secondary.slug}`">{{ secondary.name }}</RouterLink><template v-else>{{ secondary.name }}</template><b v-if="secondary.name==='Speed'" class="speed-formula">2 + <span class="agility-modifier-accent">{{ derived.aim }}</span> = {{ derived.speed }}</b><b v-else>{{ secondaryValue(secondary.name) }}</b></span><em v-if="!secondaryStats[attribute.id].length">No direct secondary stat</em></div></div><div class="attribute-rank-stack"><div class="attribute-rank-mod-row"><span class="attribute-rank-display attribute-stat-box"><span>Rank</span><b>{{ form.attributes[attribute.id] }}</b></span><span class="attribute-mod-display attribute-stat-box" :class="{'agility-modifier-box':attribute.id==='agility'}"><span>Modifier</span><b>+{{ attributeModifier(form.attributes[attribute.id]) }}</b></span></div><div class="stepper attribute-rank-stepper"><button type="button" @click="adjust(attribute.id,-1)">−</button><span>{{ form.attributes[attribute.id] }}</span><button type="button" @click="adjust(attribute.id,1)">+</button></div></div></article></div>
        </template>
        ...
        <template v-else-if="stepId==='skills'">
          ...
          <details class="creation-info-panel help-panel" :open="menusExpanded"><summary>How do Starting Skills work?</summary><div class="creation-info-body"><p>The supplied Character Creation rules grant two Skills from your Homeland at Rank 1. There is no separate pool of Skill points to spend during creation. Some selected Culture Traits grant additional Skill ranks; those are added automatically below and can raise a Skill already gained from your Homeland.</p><RouterLink to="/rules/read/attributes-skills" class="inline-rule-link">Read Attributes &amp; Skills →</RouterLink></div></details>
        ...
        <template v-else-if="stepId==='path'">
          ...
          <details class="creation-info-panel help-panel" :open="menusExpanded"><summary>Choose Your Path</summary><div class="creation-info-body"><p>This choice decides how your hero first expresses exceptional ability. Every path grants at least one Talent. Wind-Touched begins magical progression, Gifted Heart begins with a second Talent, Practiced Hand broadens Skills, and Tempered Form strengthens one Attribute.</p></div></details>
          ...
          <div class="path-choice-grid"><button v-for="path in startingPathOptions" :key="path.id" type="button" class="path-choice-card" :class="[`path-${path.id}`,{selected:pathTouched&&form.path===path.id}]" @click="choosePath(path.id)"><span class="path-choice-kicker">{{ path.kicker }}</span><strong class="path-choice-title">{{ path.title }}</strong><p>{{ path.description }}</p></button></div>
        </template>
        ...
        <template v-else-if="stepId==='path-skills'">
          ...
          <details class="creation-info-panel help-panel" :open="menusExpanded"><summary>Choose Two New Skills</summary><div class="creation-info-body"><p>Practiced Hand grants two Skills at Rank 1 in addition to your Homeland and Culture Skills. Each choice must be a Skill your character does not already know.</p></div></details>
        ...
        <template v-else-if="stepId==='path-attribute'">
          ...
          <details class="creation-info-panel help-panel" :open="menusExpanded"><summary>Choose One Attribute</summary><div class="creation-info-body"><p>Tempered Form strengthens one Attribute before you choose your Talent.</p></div></details>
        ...
        <template v-else-if="stepId==='lore'">
          ...
          <template v-if="form.loreAttunement"><article class="choice-summary lore-fluff-card"><h2>Lore of {{ form.loreAttunement }}</h2><p>{{ loreDescription(form.loreAttunement) }}</p></article><SpellCard v-if="signatureDetail" class="signature-spell-card" :title="signatureDetail.name" :cost="spellCostText(signatureDetail.name)" :lore="signatureDetail.lore" :flavor="signatureDetail.flavor" :fields="visibleRuleFields(signatureDetail.rules)" :keywords="signatureDetail.keywords" signature /></template>
        </template>
        ...
        <template v-else-if="stepId==='spells'">
          ...
          <IndependentCardColumns :items="[...form.spells,...form.invocationSpells].filter(Boolean)" class="selected-spell-stack"><template #default="{item:spell}"><SpellCard :title="spell" :cost="spellCostText(spell)" :lore="spellDetail(spell)?.lore||'Invocation'" :flavor="spellDetail(spell)?.flavor||''" :fields="visibleRuleFields(spellDetail(spell)?.rules||'')" :keywords="spellDetail(spell)?.keywords||[]" :custom="isCustomSpell(spell)" :signature="spell===signatureSpell" /></template></IndependentCardColumns>
        </template>
        ...
        <template v-else-if="stepId==='talents'">
          ...
          <details class="creation-info-panel help-panel" :open="menusExpanded"><summary>What are Talents?</summary><div class="creation-info-body"><p>Talents represent trained techniques, unusual gifts, and practiced capabilities. Some Talents require another Talent first. Requirements must be met before this character can be finished.</p></div></details>
          <div class="talent-slot-grid"><button v-for="index in requiredTalentCount()" :key="index" type="button" class="talent-slot-button" :class="form.talents[index-1]?talentCardClasses(form.talents[index-1]):[]" @click="openTalentPicker(index-1)"><span>Talent {{ index }}</span><strong>{{ form.talents[index-1]||'Choose a Talent' }}</strong></button></div>
          <div class="selected-talent-stack"><TalentCard v-for="talent in form.talents.filter(Boolean)" :key="talent" :title="canonicalTalentName(talent)" :cost="talentManaCost(talentText(talent))!==null?`${talentManaCost(talentText(talent))} Mana`:''" :flavor="structuredRule(talentText(talent)).intro" :fields="visibleRuleFields(talentText(talent))" :keywords="talentFeaturePillKeywords(talent,talentKeywords(talent))" :tone-class="talentCardClasses(talent)" :requirement="talentRequires(talent)" :requirement-met="talentRequirementMet(talent)" :class="{invalid:!talentRequirementMet(talent)}"><p v-if="!talentRequirementMet(talent)" class="invalid-option-text">Requirement not met. Select {{ talentRequires(talent) }} as the other Talent or choose a different option.</p></TalentCard></div>
        ...
        <template v-else-if="stepId==='languages'">
          ...
          <details class="creation-info-panel help-panel" :open="menusExpanded"><summary>Languages</summary><div class="creation-info-body"><p>Your Species gives you its native tongue, and Commonspeak serves as the shared trade language used between peoples. Choose one bonus language to represent travel, study, kinship, work, or another part of your character’s history.</p></div></details>
        ...
        <template v-else>
          ...
          <details class="review-section review-collapsible review-armor-section" open><summary><h2>Armor &amp; Shield</h2><span>{{ reviewArmor.length }} / 2 protective slots</span></summary><div class="review-collapsible-body"><div class="equipment-scroll-shell"><button type="button" class="equipment-scroll-button" aria-label="Scroll armor left" @click="scrollEquipment(armorSheet,-1)">‹</button><div ref="armorSheet" class="equipment-slot-sheet"><div class="equipment-slot-header armor-slot-grid"><span>Armor / Shield</span><span>Guts</span><span>Mana Syphon</span><span>Armor Penalty</span><span>Might</span><span>Wt.</span></div><div v-for="(slot,index) in armorSlots" :key="`armor-slot-${index}`" class="equipment-slot-row armor-slot-grid"><strong>{{ slot.name }}</strong><span>{{ slot.guts }}</span><span>{{ slot.mana }}</span><span>{{ slot.armorPenalty }}</span><span>{{ slot.might }}</span><span>{{ slot.weight }}</span></div></div><button type="button" class="equipment-scroll-button" aria-label="Scroll armor right" @click="scrollEquipment(armorSheet,1)">›</button></div><div class="trinket-slot-sheet"><h3>Trinkets</h3><IndependentCardColumns v-if="reviewTrinkets.length" :items="reviewTrinkets" item-key="name" class="trinket-slot-grid"><template #default="{item}"><article class="trinket-slot-card"><strong>{{ item.name }}</strong><small>{{ item.effect || item.detail || 'Passive trinket effect.' }}</small></article></template></IndependentCardColumns><p v-else class="muted">No trinkets equipped.</p></div></div></details>
          <details class="review-section review-collapsible character-sheet-content-section"><summary><h2>Skills</h2></summary><div class="review-collapsible-body"><IndependentCardColumns :items="reviewSkillEntries" item-key="name" class="review-skill-grid"><template #default="{item:skill}"><article class="review-skill-sheet-row"><div><strong>{{ skill.name }}</strong><small v-if="cultureRankForSkill(skill.name)" class="cultural-skill-pill">Cultural · {{ cultureSourcesForSkill(skill.name) }}</small></div><div class="review-skill-values"><strong>Rank {{ skill.rank }}</strong><small>Modifier +{{ skillModifier(skill.rank) }}</small></div></article></template></IndependentCardColumns></div></details>
          <details v-if="form.path==='magic'" class="review-section review-collapsible character-sheet-content-section"><summary><h2>Magic</h2></summary><div class="review-collapsible-body"><article class="review-feature-box lore full-rule-entry detail-tone-magic"><strong>Lore of {{ form.loreAttunement||'—' }}</strong><span>{{ loreDescription(form.loreAttunement)||'No Lore selected.' }}</span></article><h3 class="review-subheading">Lore Spells</h3><div class="spell-card-grid character-spell-column"><SpellCard v-for="spell in reviewLoreSpells" :key="spell" :title="spell" :cost="spellCostText(spell)" :lore="spellDetail(spell)?.lore||form.loreAttunement" :flavor="spellDetail(spell)?.flavor||''" :fields="visibleRuleFields(spellDetail(spell)?.rules||'')" :keywords="spellDetail(spell)?.keywords||[]" :custom="isCustomSpell(spell)" :signature="spell===signatureSpell" /></div><h3 class="review-subheading">Invocation Spells</h3><div class="spell-card-grid character-spell-column"><SpellCard v-for="spell in reviewInvocationSpells" :key="spell" :title="spell" :cost="spellCostText(spell)" :lore="spellDetail(spell)?.lore||'Invocation'" :flavor="spellDetail(spell)?.flavor||''" :fields="visibleRuleFields(spellDetail(spell)?.rules||'')" :keywords="spellDetail(spell)?.keywords||[]" :custom="isCustomSpell(spell)" /><p v-if="!reviewInvocationSpells.length" class="muted">No Invocation Spells selected.</p></div></div></details>
          <details class="review-section review-collapsible character-sheet-content-section"><summary><h2>Talents</h2></summary><div class="review-collapsible-body review-box-list review-talent-column"><TalentCard v-for="talent in form.talents.filter(Boolean)" :key="talent" :title="canonicalTalentName(talent)" :cost="talentManaCost(talentText(talent))!==null?`${talentManaCost(talentText(talent))} Mana`:''" :flavor="structuredRule(talentText(talent)).intro" :fields="visibleRuleFields(talentText(talent))" :keywords="talentFeaturePillKeywords(talent,talentKeywords(talent))" :tone-class="talentCardClasses(talent)" :custom="isCustomTalent(talent)" /><p v-if="!form.talents.filter(Boolean).length">No Talents selected.</p></div></details>
          <details class="review-section review-collapsible character-sheet-content-section"><summary><h2>Equipment &amp; Gear</h2><span>{{ threadpieceBalanceLabel }} — {{ carriedWeightLabel }}</span></summary><div class="review-collapsible-body"><IndependentCardColumns v-if="reviewEquipmentCards.length" :items="reviewEquipmentCards" item-key="name" class="character-equipment-card-grid"><template #default="{item}"><EquipmentCard :title="item.name" :badge="item.costText||''" :tone-class="equipmentToneClass(item)" :fields="equipmentCardFields(item)" :description="item.description||''"><template #footer><div class="character-equipment-card-footer"><small v-if="equipmentQuantity(item)>1">Quantity: {{ equipmentQuantity(item) }}</small><small v-if="item.choice">Choice: {{ item.choice }}</small><small v-if="item.attachedTo">Attached to: {{ item.attachedTo }}</small></div></template></EquipmentCard></template></IndependentCardColumns><p v-else class="muted">No additional equipment purchased.</p></div></details>
      ...
      <section ref="cultureDialog" class="modal-card culture-picker-modal" role="dialog" aria-modal="true" aria-label="Choose Culture Traits">
        ...
        <div class="culture-picker-list">
          <article v-for="trait in filteredCultureTraits" :key="`${trait.species}-${trait.name}`" class="culture-picker-row">
            ...
            <div class="keyword-pill-row"><span v-for="keyword in cultureFooterKeywords(trait.keywords,trait.species,Boolean(cultureGrant(cultureId(trait.species,trait.name))))" :key="keyword" :class="traitFooterClass(keyword)">{{ traitFooterLabel(keyword) }}</span></div>
          </article>
        </div>
    ...
    <div v-if="talentPickerOpen" class="modal-backdrop talent-picker-backdrop" @click.self="talentPickerOpen=false">
      <section ref="talentDialog" class="modal-card talent-picker-modal" role="dialog" aria-modal="true" aria-label="Choose a Talent">
        ...
        <div class="talent-picker-list"><TalentCard v-for="talent in filteredTalentPicker" :key="talent" class="talent-picker-card" :title="canonicalTalentName(talent)" :cost="talentManaCost(talentText(talent))!==null?`${talentManaCost(talentText(talent))} Mana`:''" :flavor="structuredRule(talentText(talent)).intro" :fields="visibleRuleFields(talentText(talent))" :keywords="talentFeaturePillKeywords(talent,talentKeywords(talent))" :tone-class="talentCardClasses(talent)" :custom="isCustomTalent(talent)" :requirement="talentRequires(talent)" :requirement-met="talentRequirementMet(talent)" :class="{invalid:!talentRequirementMet(talent)}"><div class="talent-picker-action"><button type="button" class="secondary-button" :class="{active:talentSelected(talent)}" :disabled="!talentSelected(talent)&&(form.talents.filter(Boolean).length>=requiredTalentCount()||!talentRequirementMet(talent))" @click="toggleTalentChoice(talent)">{{ talentSelected(talent)?'Selected':'Select' }}</button></div></TalentCard></div><div class="talent-picker-footer"><span>{{ form.talents.filter(Boolean).length }} / {{ requiredTalentCount() }} selected</span><button type="button" class="primary-button" :disabled="!talentsValid()" @click="talentPickerOpen=false">Done</button></div>
      </section>
    </div>

    <EquipmentShopModal :open="shopOpen" :items="gearShopItems" :balance-wp="wealthRemaining" :might="effectiveAttributes.might" @close="shopOpen=false" @purchase="purchaseFromSharedShop" />
  </div>
</template>
...
.creation-info-panel{border-radius:10px;border:1px solid var(--line-dark);border-top:5px solid var(--accent);background:var(--paper);box-shadow:0 2px 8px rgba(0,0,0,.05)}
.creation-info-panel>summary{min-height:42px;background:color-mix(in srgb,var(--accent) 8%,var(--paper-2));border-bottom:0}
.creation-info-panel[open]>summary{border-bottom:1px solid var(--line)}
.creation-info-panel .creation-info-body{background:var(--paper)}
...
.culture-picker-row-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}.culture-picker-row-head>div:first-child{display:grid;gap:2px}.culture-picker-row-actions{display:flex;align-items:center;gap:6px}.culture-picker-row-head strong{font-family:Georgia,'Times New Roman',serif;font-size:calc(17px + var(--font-offset))}.culture-picker-row-head small{color:var(--ink-soft)}
...
.skill-rank-values{display:grid;justify-items:end;gap:1px}.skill-rank-values b{color:var(--ink);font-weight:850}.skill-rank-values small{color:var(--accent-dark);font-size:calc(9px + var(--font-offset));font-weight:800}
...
.language-card{min-height:155px;padding:14px}.language-card p{margin:3px 0;color:var(--ink-soft);line-height:1.55}
.review-secondary-cards{grid-template-columns:repeat(4,minmax(0,1fr))}.character-detail-body{display:grid;gap:10px;min-width:0}.character-detail-body .review-summary-grid{margin-top:0}.review-collapsible{padding:0;overflow:hidden;min-width:0;max-width:100%}.review-collapsible>summary{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:11px 12px;cursor:pointer;background:var(--paper-2)}.review-collapsible>summary h2{margin:0}.review-collapsible>summary span{color:var(--ink-soft)}.review-collapsible-body{padding:10px 12px;border-top:1px solid var(--line)}.review-collapsible-body p{margin:5px 0}
.creation-example{border-left:4px solid #b06c27;border-right:4px solid #b06c27;background:color-mix(in srgb,#dca96f 18%,var(--paper-2))}
...
.custom-data-toggle{display:flex;align-items:center;justify-content:space-between;gap:12px;min-height:50px;padding:7px 9px;border:1px solid var(--line);border-radius:8px;background:var(--paper-2)}.custom-data-toggle>span:first-child{display:grid;gap:2px}.custom-data-toggle small{color:var(--ink-soft);font-size:calc(8.5px + var(--font-offset))}.campaign-custom-stack{display:grid;gap:6px}.custom-content-badge{display:inline-flex;align-items:center;margin-left:5px;padding:2px 5px;border:1px solid #7b4f8d;border-radius:999px;background:#efe5f4;color:#60336f;font-family:Inter,ui-sans-serif,system-ui,sans-serif;font-size:8px;font-weight:900;letter-spacing:.06em;vertical-align:middle}.creator-name-campaign-grid{align-items:end}

/* Canonical creator structures. */
.attribute-sheet-row{--attribute-tone:#a06b2c;grid-template-columns:minmax(0,1fr) minmax(220px,.72fr);align-items:start}.attribute-sheet-row.attribute-agility{--attribute-tone:#46779a}.attribute-sheet-row.attribute-might{--attribute-tone:#9b543e}.attribute-sheet-row.attribute-hide{--attribute-tone:#527934}.attribute-sheet-row.attribute-lore{--attribute-tone:#6d5293}.attribute-sheet-row.attribute-bravery{--attribute-tone:#a06b2c}.attribute-sheet-copy{gap:7px}.attribute-sheet-secondary{margin-top:3px}.attribute-sheet-secondary>span,.attribute-stat-box{display:grid;align-items:center;gap:8px;min-height:44px;padding:7px 9px;border:1px solid var(--line);border-left:4px solid var(--attribute-tone);border-radius:7px;background:var(--paper-2)}.attribute-sheet-secondary>span{grid-template-columns:1fr auto;text-align:left;flex:0 1 120px;font-size:calc(9px + var(--font-offset))}.attribute-stat-box{grid-template-columns:1fr;justify-items:center;text-align:center}.attribute-sheet-secondary>span b,.attribute-stat-box b{font-size:calc(13px + var(--font-offset));line-height:1}.attribute-sheet-secondary>span .speed-formula{font-size:calc(12px + var(--font-offset));white-space:nowrap}.attribute-sheet-secondary>span small{font-size:calc(8px + var(--font-offset));color:var(--ink-soft)}.attribute-stat-box>span{color:var(--ink);font-size:calc(9px + var(--font-offset));font-weight:850}.attribute-rank-stack{align-self:center}.attribute-rank-mod-row{margin-bottom:7px}.attribute-rank-stepper{margin-inline:auto}.attribute-rank-stepper>span{min-width:34px;text-align:center}.skill-source-pills{display:flex;align-items:center;gap:4px;flex-wrap:wrap}.homeland-skill-pill{display:inline-flex;padding:2px 6px;border:1px solid var(--accent);border-radius:999px;background:var(--accent-wash);color:var(--accent-dark);font-size:calc(8px + var(--font-offset));font-weight:850}.currency-grid-four{grid-template-columns:repeat(4,minmax(0,1fr))}.starting-equipment-parent .starting-equipment-panel{margin-top:10px}.equipment-quantity-control{display:grid;grid-template-columns:34px 34px 34px;align-items:center;justify-content:end;gap:4px}.equipment-quantity-control strong{text-align:center}.equipment-quantity-control .compact-action{min-width:34px;padding-inline:0}.purchased-gear-list .list-row{align-items:center}.purchased-gear-actions{display:flex;align-items:center;gap:5px;justify-content:flex-end}.purchased-gear-actions .active{border-color:var(--accent);background:var(--accent-wash)}
@media(max-width:680px){.attribute-sheet-row{grid-template-columns:1fr}.attribute-rank-stack{width:100%;min-width:0}.currency-grid-four{grid-template-columns:repeat(2,minmax(0,1fr))}.purchased-gear-list .list-row{align-items:stretch}.equipment-quantity-control{justify-content:start}.purchased-gear-actions{justify-content:flex-start}}

.agility-modifier-box b,.speed-formula .agility-modifier-accent{color:var(--danger)}
