...
const gearTabs=['All','Weapons','Weapon Qualities','Armor & Shields','Traveler’s Gear','Field Kits','Consumables','Spellcasting Implements','Accessories','Tools'] as const
function gearTabFor(item:typeof gearShopItems[number]){
  ...
  if(['Scriptweave Book','Totem','Charm','Len-stone Arcanum','Cloak of Windweave'].includes(item.name))return'Spellcasting Implements'
  if(['Quickdraw Quiver','Featherwind Bolt-Case','Wristloop','Sharpening Stone','Journey Knot'].includes(item.name))return'Accessories'
  if(['Artisan','Climber','Disguise','Forgery','Navigator','Poisoner','Herbalist','Infusion','Thieves’'].includes(item.name))return'Field Kits'
  if(['Shovel','Mallet','Hatchet','Hand Saw','Trowel','Pry Bar'].includes(item.name))return'Tools'
  return'Traveler’s Gear'
}
const weaponQualityRows=computed(()=>{
  const qualityTable=ruleSourceDocuments.weapons.sections.flatMap(section=>section.blocks).find(block=>block.type==='table'&&block.rows?.[0]?.some(cell=>/quality/i.test(cell)))
  if(!qualityTable||qualityTable.type!=='table')return[] as {name:string;effect:string}[]
  return qualityTable.rows.slice(1).filter(row=>row[0]).map(row=>({name:row[0],effect:row.slice(1).filter(Boolean).join(' — ')}))
})
const filteredGear=computed(()=>{
  if(gearTab.value==='Weapon Qualities')return[]
...
function gearShopSubtitle(item:typeof gearShopItems[number]){const cost=`${adjustedGearCostNp(item)} NP`;return gearTab.value==='All'?`${gearTabFor(item)} · ${cost}`:cost}
...
function addEquipment(item:typeof gearShopItems[number]){const costNp=adjustedGearCostNp(item);if(costNp>wealthRemaining.value)return;const choices=gearChoices(item);const choice=gearChoice.value[item.name]||'';if(choices.length&&!choice)return;form.equipment.push({name:item.name,costSp:item.costSp,costNp,category:item.category,detail:gearDisplayDetail(item),effect:gearEffect(item),choice:choice||undefined})}
...
function effectiveMana(name:string){
  const detail=spellDetail(name); if(!detail||detail.manaCost===null)return null
...
<template>
  <div class="app-page creator-page">
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
          ...
          <div class="species-choice-layout">
            <div class="species-choice-copy">
              ...
              <template v-if="selectedSpeciesData">
                ...
                <details class="creation-info-panel trait-panel species-trait-panel"><summary>Heritage Traits</summary><div class="creation-info-body trait-stack"><article v-for="trait in selectedSpeciesTraits" :key="trait.name" class="trait-card species-trait-card"><div class="trait-card-head"><h3>{{ trait.name }} <small v-if="'custom' in trait&&trait.custom" class="custom-content-badge">CUSTOM</small></h3><div class="trait-title-costs"><span v-for="type in abilityTypeKeywords(trait.keywords)" :key="type" class="ability-cost-pill">{{ type }}</span><span v-if="manaCostFromText(trait.text)!==null" class="mana-badge">{{ manaCostFromText(trait.text) }} Mana</span></div></div><p v-if="structuredRule(trait.text).intro" class="rule-flavor">{{ structuredRule(trait.text).intro }}</p><div v-if="visibleRuleFields(trait.text).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(trait.text)" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cardKeywords(trait.keywords,'Heritage',form.species)" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article></div></details>
                <details class="creation-info-panel trait-panel culture-trait-panel"><summary>Culture Traits</summary><div class="creation-info-body"><p>Your Species begins with these two Culture Traits. In Step 2 you may keep them or exchange them for Culture Traits from other peoples.</p><div class="trait-stack"><article v-for="trait in selectedSpeciesData.cultureTraits" :key="trait.name" class="trait-card culture-trait-card"><div class="trait-card-head"><h3>{{ trait.name }} <small v-if="selectedSpeciesData.custom" class="custom-content-badge">CUSTOM</small></h3><div class="trait-title-costs"><span v-for="type in abilityTypeKeywords(trait.keywords)" :key="type" class="ability-cost-pill">{{ type }}</span><span v-if="manaCostFromText(trait.text)!==null" class="mana-badge">{{ manaCostFromText(trait.text) }} Mana</span></div></div><p v-if="structuredRule(trait.text).intro" class="rule-flavor">{{ structuredRule(trait.text).intro }}</p><div v-if="visibleRuleFields(trait.text).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(trait.text)" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cardKeywords(trait.keywords,'Cultural',form.species)" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article></div></div></details>
              </template>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="stepId==='spark'">
          ...
          <details class="creation-info-panel help-panel" open><summary>What is a Spark &amp; Deed?</summary><div class="creation-info-body"><p>A Spark is the personality archetype that describes what tends to move your hero into action. Each Spark has two keywords that describe its nature.</p><p><strong>Deeds</strong> are meaningful acts completed during play — moments of courage, wit, compassion, discovery, or conviction that shape who your hero becomes. A completed Deed grants Experience based on its challenge and impact. If the Deed shares a keyword with your Spark, it grants <strong>+3 bonus XP</strong> in addition to the normal reward.</p><RouterLink to="/rules/read/sparks-deeds" class="inline-rule-link">Read Sparks &amp; Deeds →</RouterLink></div></details><article class="deed-example-card"><span class="eyebrow">EXAMPLE DEED FOR {{ form.spark.toUpperCase() }}</span><h2>{{ currentDeedExample.name }}</h2><p>{{ currentDeedExample.objective }}</p><div class="deed-example-meta"><span><strong>Reward</strong> {{ currentDeedExample.reward }}</span><div class="keyword-pill-row"><span v-for="keyword in currentDeedExample.keywords" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></div></article>
          ...
          <article class="choice-summary spark-description-card compact-spark-preview"><h2>{{ form.spark }}</h2><p>{{ currentSpark?.description }}</p><div class="keyword-pill-row"><span v-for="keyword in singleKeywords(currentSpark?.keywords||[])" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article>
        </template>
        ...
        <template v-else-if="stepId==='homeland'">
          ...
          <article v-else-if="form.homeland" class="choice-summary homeland-description-card"><h2>{{ form.homeland }}</h2><p>{{ homelandDetail?.description }}</p><div class="homeland-skill-copy"><strong>Skills</strong><div class="keyword-pill-row"><span v-for="skill in homelandCoreSkills" :key="skill" class="keyword-pill">{{ skill }}</span></div><small v-if="homelandOptionalSkills.length"><strong>Optional</strong></small><div v-if="homelandOptionalSkills.length" class="keyword-pill-row"><span v-for="skill in homelandOptionalSkills" :key="skill" class="keyword-pill alternate-pill">{{ skill }}</span></div></div></article>
        </template>
        ...
        <template v-else-if="stepId==='attributes'">
          ...
          <div class="attribute-editor attribute-sheet-editor"><article v-for="attribute in attributes" :key="attribute.id" class="attribute-sheet-row"><div class="attribute-sheet-copy"><div class="attribute-sheet-title"><strong>{{ attribute.name }}</strong></div><small>{{ attribute.description }}</small><div class="attribute-sheet-secondary"><span v-for="secondary in secondaryStats[attribute.id]" :key="secondary.name"><RouterLink v-if="secondary.slug" :to="`/rules/read/${secondary.slug}`">{{ secondary.name }}</RouterLink><template v-else>{{ secondary.name }}</template><b>{{ secondary.name==='Speed'?derived.speed:secondary.name==='Aim'?derived.aim:secondary.name==='Mettle'?derived.mettle:secondary.name==='Power'?derived.power:secondary.name==='Ward'?derived.ward:secondary.name==='Guts'?derived.guts:derived.control }}</b></span><em v-if="!secondaryStats[attribute.id].length">No direct secondary stat</em></div></div><div class="attribute-rank-stack"><div class="attribute-rank-mod-row"><span class="attribute-rank-display"><small>Rank</small><b>{{ form.attributes[attribute.id] }}</b></span><span class="attribute-mod-display"><small>Modifier</small><b>+{{ attributeModifier(form.attributes[attribute.id]) }}</b></span></div><div class="stepper"><button type="button" @click="adjust(attribute.id,-1)">−</button><span>{{ form.attributes[attribute.id] }}</span><button type="button" @click="adjust(attribute.id,1)">+</button></div></div></article></div>
        </template>
        ...
        <template v-else-if="stepId==='spells'">
          ...
          <div class="selected-spell-stack"><article v-for="spell in [...form.spells,...form.invocationSpells].filter(Boolean)" :key="spell" class="spell-detail-card" :class="spellLoreClass(spell)"><div class="spell-detail-head"><div><span class="eyebrow">{{ spellLore(spell) }}</span><h2>{{ spell }} <small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></h2></div><div class="trait-title-costs"><span v-for="type in abilityTypeKeywords(spellDetail(spell)?.keywords||[])" :key="type" class="ability-cost-pill">{{ type }}</span><span class="mana-badge"><template v-if="spellDetail(spell)?.manaCost!==null">{{ effectiveMana(spell) }} Mana</template><template v-else>Variable</template></span></div></div><p class="rule-flavor">{{ spellDetail(spell)?.flavor }}</p><div class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cardKeywords(spellDetail(spell)?.keywords||[])" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article></div>
        </template>
        ...
        <template v-else-if="stepId==='equipment'">
          ...
          <details class="creation-info-panel help-panel" open><summary>Starting Equipment &amp; Currency</summary><div class="creation-info-body"><p>Trade across Anthro Mundas commonly uses Ancient fasteners called <strong>Threadpieces</strong>. Washer pieces (wp) cover everyday purchases, Nut pieces (np) local trade, Screw pieces (sp) most adventuring commerce, and Bolt pieces (bp) major debts and purchases.</p><div class="currency-grid currency-grid-three"><span><strong>10 wp</strong><small>= 1 np</small></span><span><strong>5 np</strong><small>= 1 sp</small></span><span><strong>2 sp</strong><small>= 1 bp</small></span></div></div></details>
          <section class="starting-equipment-panel"><div class="adventure-kit-bar"><div><strong>Adventure Kit - Starting Equipment Package</strong></div><div class="kit-return-control"><span>Sell</span><label class="switch"><input v-model="form.adventureKit" type="checkbox" :true-value="false" :false-value="true"/><span></span></label></div></div><div v-if="form.adventureKit" class="adventure-kit-contents"><span>Bedroll &amp; Groundsheet</span><span>Traveler’s Cloak</span><span>2× Torches</span><span>Reed Flask</span><span>2× Trail Rations</span><span>Traveler’s Pack</span><span>Fire-Starting Kit</span></div><div class="wealth-balance-field"><span>Threadpieces</span><div class="threadpiece-breakdown"><span><b>BP</b></span><span><b>{{ threadpieceBreakdown.sp }}</b> SP</span><span><b>{{ threadpieceBreakdown.np }}</b> NP</span><span><b>{{ threadpieceBreakdown.wp }}</b> WP</span></div></div></section>
          <button type="button" class="secondary-button wide" @click="shopOpen=true">Equipment &amp; Gear</button>
          <section v-if="form.equipment.length" class="purchased-equipment-section"><h2>Purchased Equipment &amp; Gear</h2><div class="purchased-gear-list"><article v-for="(item,index) in form.equipment" :key="`${item.name}-${index}`" class="list-row"><span class="list-row-copy"><strong class="list-row-title">{{ item.name }}</strong><small class="list-row-subtitle">{{ item.category }} · {{ item.costNp ?? adjustedGearCostNp(item) }} NP</small></span><div class="purchased-item-detail"><small v-if="item.choice">Choice: {{ item.choice }}</small><label v-if="equipmentAttachTargets(item).length" class="attachment-control">Attach / Apply (optional)<select class="field-control" :value="item.attachedTo||''" @change="setAttachment(index,$event)"><option value="">Not attached</option><option v-for="target in equipmentAttachTargets(item)" :key="target" :value="target">{{ target }}</option></select></label></div><button type="button" class="secondary-button compact-action" @click="removeEquipment(index)">Remove</button></article></div></section>
        </template>
        ...
        <template v-else-if="stepId==='languages'">
          ...
          <div class="language-choice-grid compact-language-grid"><article class="choice-summary language-card species-language-card" :data-species="form.species"><h2>{{ nativeLanguage }}</h2><div class="keyword-pill-row"><span class="keyword-pill">{{ form.species }} Language</span></div><p>{{ languageDescription(nativeLanguage) }}</p></article><article class="choice-summary language-card common-language-card"><h2>Commonspeak</h2><p>{{ languageDescription('Commonspeak') }}</p></article><article class="choice-summary language-card additional-language-card"><h2>Additional Language</h2><select v-model="form.additionalLanguage" class="field-control"><option value="">Select Language</option><option v-for="language in languageOptions" :key="language" :value="language">{{ language }}</option></select><p v-if="form.additionalLanguage">{{ languageDescription(form.additionalLanguage) }}</p></article></div>
        </template>
        ...
        <template v-else>
          ...
          <div class="field-grid two review-identity-edit"><label class="field-label review-name-field">Character Name<input v-model="form.name" class="field-control" placeholder="Character name" /></label><label class="field-label review-name-field">Campaign<input v-model="form.campaignName" class="field-control" placeholder="Campaign name" /></label></div>
          <div class="review-grid review-summary-grid"><article><span>Species</span><strong>{{ form.species||'Not selected' }}</strong><small>{{ nativeLanguage }}</small></article><article><span>Spark</span><strong>{{ form.spark }}</strong></article><article><span>Homeland</span><strong>{{ isCustomHomeland?(form.customHomelandName||'Custom Homeland'):form.homeland }}</strong></article><article><span>Oath</span><strong>{{ form.oath }}</strong></article><article><span>Faith</span><strong>{{ form.faith }}</strong></article><article><span>Path</span><strong>{{ selectedPathName }}</strong><small>{{ form.path==='magic'?'Magic Level 1 + 1 Talent':'2 Talents' }}</small></article><article class="review-wide-field"><span>Languages</span><strong>{{ languages.join(' · ') }}</strong></article></div>
          <details class="review-section review-collapsible character-sheet-attributes" open><summary><h2>Attributes</h2><span>Ranks &amp; derived stats</span></summary><div class="review-collapsible-body"><div class="review-attribute-cards single-row-attributes"><article v-for="attribute in attributes" :key="attribute.id"><strong>{{ attribute.name }}</strong><span>Rank {{ form.attributes[attribute.id] }}</span><small>Modifier +{{ attributeModifier(form.attributes[attribute.id]) }}</small></article></div><div class="review-secondary-strip"><article><span>Speed</span><strong>{{ derived.speed }}</strong></article><article><span>Aim</span><strong>{{ derived.aim }}</strong></article><article><span>Mettle</span><strong>{{ derived.mettle }}</strong></article><article><span>Ward</span><strong>{{ derived.ward }}</strong></article><article><span>Control</span><strong>{{ derived.control }}</strong></article><article><span>Power</span><strong>{{ derived.power }}</strong></article><article><span>Guts</span><strong>{{ reviewGuts }}</strong><small v-if="gearGutsBonus">{{ derived.guts }} + {{ gearGutsBonus }} gear</small></article><article class="review-mana-stat"><span>Mana</span><small>Max / Round</small><strong>{{ startingMana }} / +{{ manaPerRound }}</strong></article></div></div></details>
          <details class="review-section review-collapsible review-weapons-section" open><summary><h2>Weapons</h2><span>{{ reviewWeapons.length }} equipped</span></summary><div class="review-collapsible-body equipment-slot-sheet"><div class="equipment-slot-header weapon-slot-grid"><span>Weapon</span><span>Damage</span><span>Range</span><span>Properties / Notes</span><span>Wt.</span></div><div v-for="(slot,index) in weaponSlots" :key="`weapon-slot-${index}`" class="equipment-slot-row weapon-slot-grid"><strong>{{ slot.name }}</strong><span>{{ slot.damage }}</span><span>{{ slot.range }}</span><span>{{ slot.properties }}</span><span>{{ slot.weight }}</span></div></div></details>
          <details class="review-section review-collapsible review-armor-section" open><summary><h2>Armor &amp; Shields</h2><span>{{ reviewArmor.length }} equipped</span></summary><div class="review-collapsible-body equipment-slot-sheet"><div class="equipment-slot-header armor-slot-grid"><span>Armor / Shield</span><span>Guts</span><span>Mana Syphon</span><span>Stealth</span><span>Might</span><span>Wt.</span></div><div v-for="(slot,index) in armorSlots" :key="`armor-slot-${index}`" class="equipment-slot-row armor-slot-grid"><strong>{{ slot.name }}</strong><span>{{ slot.guts }}</span><span>{{ slot.mana }}</span><span>{{ slot.stealth }}</span><span>{{ slot.might }}</span><span>{{ slot.weight }}</span></div></div></details>
          <details class="review-section review-collapsible"><summary><h2>Skills</h2><span>{{ Object.keys(skillRanks).length }} recorded</span></summary><div class="review-collapsible-body"><div class="review-skill-grid"><article v-for="(rank,skill) in skillRanks" :key="skill" class="review-skill-sheet-row"><div><strong>{{ skill }}</strong><small v-if="cultureRankForSkill(skill)" class="cultural-skill-pill">Cultural · {{ cultureSourcesForSkill(skill) }}</small></div><div class="review-skill-values"><strong>Rank {{ rank }}</strong><small>Modifier +{{ skillModifier(rank) }}</small></div></article></div></div></details>
          <details v-if="form.path==='magic'" class="review-section review-collapsible" open><summary><h2>Magic</h2><span>{{ form.loreAttunement||'Not selected' }} · {{ startingMana }} Mana</span></summary><div class="review-collapsible-body"><article class="review-feature-box lore"><strong>Lore of {{ form.loreAttunement||'—' }}</strong><span>{{ loreDescription(form.loreAttunement)||'No Lore selected.' }}</span></article><h3 class="review-subheading">Lore Spells</h3><div class="review-spell-column"><article v-for="spell in reviewLoreSpells" :key="spell" class="review-feature-box spell full-rule-entry" :class="spellLoreClass(spell)"><header><strong>{{ spell }}</strong><small v-if="effectiveMana(spell)!==null">{{ effectiveMana(spell) }} Mana</small></header><span>{{ spell===signatureSpell?'Signature Spell · ':'' }}{{ spellLore(spell) }}</span><p class="rule-flavor"><em>{{ spellDetail(spell)?.flavor }}</em></p><div v-if="visibleRuleFields(spellDetail(spell)?.rules||'').length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div></article></div><h3 class="review-subheading">Invocation Spells</h3><div class="review-spell-column"><article v-for="spell in reviewInvocationSpells" :key="spell" class="review-feature-box spell full-rule-entry" :class="spellLoreClass(spell)"><header><strong>{{ spell }}</strong><small v-if="effectiveMana(spell)!==null">{{ effectiveMana(spell) }} Mana</small></header><span>{{ spellLore(spell) }}</span><p class="rule-flavor"><em>{{ spellDetail(spell)?.flavor }}</em></p><div v-if="visibleRuleFields(spellDetail(spell)?.rules||'').length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div></article><p v-if="!reviewInvocationSpells.length" class="muted">No Invocation Spells selected.</p></div></div></details>
          <details class="review-section review-collapsible"><summary><h2>Talents</h2><span>{{ form.talents.filter(Boolean).length }} selected</span></summary><div class="review-collapsible-body review-box-list"><article v-for="talent in form.talents.filter(Boolean)" :key="talent" class="review-feature-box talent full-rule-entry"><header><strong>{{ canonicalTalentName(talent) }}</strong><small>{{ talentCategory(talent) }}</small></header><p class="rule-flavor"><em>{{ structuredRule(talentText(talent)).intro }}</em></p><div v-if="visibleRuleFields(talentText(talent)).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(talentText(talent))" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div></article><p v-if="!form.talents.filter(Boolean).length">No Talents selected.</p></div></details>
          <details class="review-section review-collapsible"><summary><h2>Equipment &amp; Gear</h2><span>{{ form.equipment.filter(item=>item.category!=='Weapon'&&item.category!=='Armor & Shield').length }} items</span></summary><div class="review-collapsible-body"><div v-if="form.equipment.filter(item=>item.category!=='Weapon'&&item.category!=='Armor & Shield').length" class="review-equipment-grid"><article v-for="(item,index) in form.equipment.filter(item=>item.category!=='Weapon'&&item.category!=='Armor & Shield')" :key="`${item.name}-${index}`"><strong>{{ item.name }}</strong><small>{{ item.category }} · {{ item.costNp ?? adjustedGearCostNp(item) }} NP</small><p v-if="item.detail">{{ item.detail }}</p><p v-if="item.effect">{{ item.effect }}</p><small v-if="item.choice">Choice: {{ item.choice }}</small><small v-if="item.attachedTo">Attached to: {{ item.attachedTo }}</small></article></div><p v-else class="muted">No additional equipment purchased.</p></div></details>
      ...
      <section ref="cultureDialog" class="modal-card culture-picker-modal" role="dialog" aria-modal="true" aria-label="Choose Culture Traits">
        ...
        <div class="culture-picker-list">
          <article v-for="trait in filteredCultureTraits" :key="`${trait.species}-${trait.name}`" class="culture-picker-row">
            ...
            <div class="keyword-pill-row"><span v-for="keyword in cardKeywords(trait.keywords,'Cultural',trait.species)" :key="keyword" class="keyword-pill">{{ keyword }}</span></div>
          </article>
        </div>
    ...
    <div v-if="shopOpen" class="modal-backdrop gear-picker-backdrop" @click.self="shopOpen=false"><section ref="gearDialog" class="modal-card gear-shop-modal" role="dialog" aria-modal="true" aria-label="Equipment and Gear"><div class="modal-head"><div><span class="eyebrow">EQUIPMENT &amp; GEAR</span><h2>Equipment &amp; Gear</h2></div><button type="button" class="icon-button" @click="shopOpen=false">×</button></div><div class="gear-tabs" role="tablist"><button v-for="tab in gearTabs" :key="tab" type="button" :class="{active:gearTab===tab}" @click="gearTab=tab">{{ tab }}</button></div><div class="rules-search gear-search"><span aria-hidden="true">⌕</span><input v-model="shopSearch" placeholder="Search equipment…" /></div><div v-if="gearTab==='Weapon Qualities'" class="gear-shop-list weapon-quality-list"><article v-for="quality in weaponQualityRows" :key="quality.name" class="gear-shop-row weapon-quality-row"><div class="gear-shop-copy"><div class="gear-shop-title"><strong>{{ quality.name }}</strong><small>Weapon Quality</small></div><p class="gear-profile">{{ quality.effect }}</p></div></article></div><div v-else class="gear-shop-list"><article v-for="item in filteredGear" :key="`${item.category}-${item.name}`" class="gear-shop-row"><div class="gear-shop-copy"><div class="gear-shop-title"><strong>{{ item.name }}</strong><small>{{ gearShopSubtitle(item) }}</small></div><p class="gear-profile">{{ gearDisplayDetail(item) }}</p><label v-if="gearChoices(item).length" class="field-label gear-choice-field">Choose {{ item.name }} option<select v-model="gearChoice[item.name]" class="field-control"><option value="">Select Option</option><option v-for="choice in gearChoices(item)" :key="choice" :value="choice">{{ choice }}</option></select></label></div><button type="button" class="secondary-button compact-action" :disabled="adjustedGearCostNp(item)>wealthRemaining||(gearChoices(item).length>0&&!gearChoice[item.name])" @click="addEquipment(item)">Add</button></article></div></section></div>
  </div>
</template>
