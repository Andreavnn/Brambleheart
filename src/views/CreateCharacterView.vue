...
import { characterSheetArmorProfile, characterSheetWeaponProfile, derivedStats, equippedProtectiveGear, equipmentAttachmentTargets, equipmentControlBonus, equipmentGutsBonus, equipmentMagicRegenBonus, equipmentManaSyphon, equipmentSpellManaReduction, magicResources, normalizeSkillName, primaryTrinketGear, rankModifier, secondaryTrinketGear, structuredRule, visibleRuleFields, equipmentArmorPenalty, equipmentSpeedPenalty } from '../rules/rulesEngine'
...
import { canEquipProtectiveEquipment, canEquipSecondTrinket, characterStatus, loadCharacters, setProtectiveEquipmentEquipped, setTrinketEquipmentEquipped, upsertCharacter, type AttributeRanks, type CharacterRecord, type PurchasedEquipment } from '../services/characters'
...
function addEquipment(item:typeof gearShopItems[number]){
  ...
  const occupied=new Set(form.equipment.filter(candidate=>isTrinketGear(candidate)&&candidate.equipped===true).map(candidate=>candidate.trinketSlot))
  const trinketSlot=trinket?(!occupied.has('primary')?'primary':!occupied.has('secondary')?'secondary':undefined):undefined
  const equipped=kind?!form.equipment.some(candidate=>protectiveGearKind(candidate)===kind&&candidate.equipped===true):trinket?Boolean(trinketSlot):undefined
  form.equipment.push({name:item.name,costWp,costPaidWp:costWp,costSp:costWp/WP_PER_SP,costNp:costWp/WP_PER_NP,category:item.category,detail:gearDisplayDetail(item),effect:gearEffect(item),choice:choice||undefined,quantity:1,statBonuses:item.statBonuses,equipped,trinketSlot})
}
...
function removeEquipment(index:number){
  const removed=form.equipment[index];const kind=removed?protectiveGearKind(removed):null;const wasEquipped=Boolean(removed?.equipped);const removedSlot=removed&&isTrinketGear(removed)&&removed.equipped?removed.trinketSlot:undefined
  ...
  if(removedSlot){const replacement=form.equipment.findIndex(item=>isTrinketGear(item)&&item.equipped!==true);if(replacement>=0)form.equipment=setTrinketEquipmentEquipped(form.equipment,replacement,removedSlot)}
}
...
function setTrinketSlot(index:number,slot:'primary'|'secondary'){
...
const primaryReviewTrinket=computed(()=>primaryTrinketGear(form.equipment))
const secondaryReviewTrinket=computed(()=>secondaryTrinketGear(form.equipment))
