...
import { canEquipProtectiveEquipment, canEquipTrinketInShieldSlot, characterStatus, loadCharacters, setProtectiveEquipmentEquipped, setTrinketEquipmentEquipped, upsertCharacter, type AttributeRanks, type CharacterRecord, type PurchasedEquipment } from '../services/characters'
...
const gearTabsBar=ref<HTMLElement|null>(null)
...
function addEquipment(item:typeof gearShopItems[number]){
  ...
  const hasDedicatedTrinket=form.equipment.some(candidate=>isTrinketGear(candidate)&&candidate.equipped===true&&candidate.trinketSlot!=='shield')
  const equipped=kind?!form.equipment.some(candidate=>protectiveGearKind(candidate)===kind&&candidate.equipped===true)&&!(kind==='shield'&&form.equipment.some(candidate=>isTrinketGear(candidate)&&candidate.equipped===true&&candidate.trinketSlot==='shield')):trinket?!hasDedicatedTrinket:undefined
...
function removeEquipment(index:number){
  const removed=form.equipment[index];const kind=removed?protectiveGearKind(removed):null;const wasEquipped=Boolean(removed?.equipped);const removedDedicated=Boolean(removed&&isTrinketGear(removed)&&removed.equipped&&removed.trinketSlot!=='shield')
...
function setTrinketSlot(index:number,slot:'trinket'|'shield'){
...
<template>
  <div class="app-page creator-page">
    ...
    <div v-if="shopOpen" class="modal-backdrop gear-picker-backdrop" @click.self="shopOpen=false"><section ref="gearDialog" class="modal-card gear-shop-modal" role="dialog" aria-modal="true" aria-label="Equipment and Gear"><div class="modal-head"><div><span class="eyebrow">EQUIPMENT &amp; GEAR</span><h2>Equipment &amp; Gear</h2></div><button type="button" class="icon-button" @click="shopOpen=false">×</button></div><div class="gear-tab-scroll-shell"><button type="button" class="gear-tab-scroll-button" aria-label="Scroll equipment categories left" @click="scrollEquipment(gearTabsBar,-1)">‹</button><div ref="gearTabsBar" class="gear-tabs" role="tablist"><button v-for="tab in gearTabs" :key="tab" type="button" :class="{active:gearTab===tab}" @click="gearTab=tab">{{ tab }}</button></div><button type="button" class="gear-tab-scroll-button" aria-label="Scroll equipment categories right" @click="scrollEquipment(gearTabsBar,1)">›</button></div><div class="rules-search gear-search"><span aria-hidden="true">⌕</span><input v-model="shopSearch" placeholder="Search equipment…" /></div><div class="gear-shop-list"><article v-for="item in filteredGear" :key="`${item.category}-${item.name}`" class="gear-shop-row"><div class="gear-shop-copy"><div class="gear-shop-title"><strong>{{ item.name }}</strong><small>{{ gearShopSubtitle(item) }}</small></div><p v-if="gearDescription(item)" class="gear-description">{{ gearDescription(item) }}</p><p v-if="gearDisplayDetail(item)" class="gear-profile">{{ gearDisplayDetail(item) }}</p><div v-if="weaponQualityPills(item).length" class="keyword-pill-row gear-quality-pills"><span v-for="quality in weaponQualityPills(item)" :key="quality" class="keyword-pill">{{ quality }}</span></div><p v-if="gearEffect(item)" class="gear-effect"><strong>Effect:</strong> {{ gearEffect(item) }}</p><p v-if="item.category==='Armor & Shield'&&gearMightRequirement(item)>effectiveAttributes.might" class="gear-requirement-warning">Requires Might {{ gearMightRequirement(item) }}+</p><label v-if="gearChoices(item).length" class="field-label gear-choice-field">Choose {{ item.name }} option<select v-model="gearChoice[item.name]" class="field-control"><option value="">Select Option</option><option v-for="choice in gearChoices(item)" :key="choice" :value="choice">{{ choice }}</option></select></label></div><button type="button" class="secondary-button compact-action" :disabled="!canAddEquipment(item)" @click="addEquipment(item)">Add</button></article></div></section></div>
  </div>
</template>
