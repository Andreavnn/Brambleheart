...
const magicResourceValues=computed(()=>magicResources(form.attributes,magicLevel.value))
const manaPerRound=computed(()=>magicResourceValues.value.manaPerRound)
...
const reviewGuts=computed(()=>derivedStats(form.attributes,gearGutsBonus.value,gearControlBonus.value).guts)
const reviewControl=computed(()=>derivedStats(form.attributes,gearGutsBonus.value,gearControlBonus.value).control)
...
const secondaryStats:Record<AttributeId,Array<{name:string;formula:string;description:string;slug?:string}>>={
  ...
  might:[
    {name:'Mettle',formula:'Might Modifier',description:'Mettle is the stat used by melee Strike rolls.',slug:'to-strike'},
    {name:'Fury',formula:'Might Rank',description:'Fury adds to physical damage where a rule calls for it.',slug:'attributes-skills'},
  ],
  hide:[
  ...
  lore:[
    ...
    {name:'Power',formula:'Lore Rank',description:'Power adds to spell damage where a rule calls for it.',slug:'attributes-skills'},
  ],
  bravery:[
    {name:'Heart',formula:'Bravery Rank',description:'Heart determines Mana restored at the start of each round.',slug:'rounds-turns'},
    {name:'Inspiration',formula:'Bravery Modifier',description:'Inspiration contributes to the Mana Pool.',slug:'rounds-turns'},
  ],
}
function secondaryValue(name:string){
  const values:Record<string,number>={Speed:derived.value.speed,Aim:derived.value.aim,Mettle:derived.value.mettle,Fury:derived.value.fury,Ward:derived.value.ward,Guts:derived.value.guts,Control:derived.value.control,Power:derived.value.power,Heart:derived.value.heart,Inspiration:derived.value.inspiration}
...
function gearShopSubtitle(item:typeof gearShopItems[number]){const cost=item.costText.toUpperCase();return gearTab.value==='All'?`${gearTabFor(item)} · ${cost}`:cost}
    ...
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
        ...
        <template v-else-if="stepId==='attributes'">
          ...
          <div class="attribute-editor attribute-sheet-editor"><article v-for="attribute in attributes" :key="attribute.id" class="attribute-sheet-row"><div class="attribute-sheet-copy"><div class="attribute-sheet-title"><strong>{{ attribute.name }}</strong></div><small>{{ attribute.description }}</small><div class="attribute-sheet-secondary"><span v-for="secondary in secondaryStats[attribute.id]" :key="secondary.name"><RouterLink v-if="secondary.slug" :to="`/rules/read/${secondary.slug}`">{{ secondary.name }}</RouterLink><template v-else>{{ secondary.name }}</template><b>{{ secondaryValue(secondary.name) }}</b></span><em v-if="!secondaryStats[attribute.id].length">No direct secondary stat</em></div></div><div class="attribute-rank-stack"><div class="attribute-rank-mod-row"><span class="attribute-rank-display attribute-stat-box"><span>Rank</span><b>{{ form.attributes[attribute.id] }}</b></span><span class="attribute-mod-display attribute-stat-box"><span>Modifier</span><b>+{{ attributeModifier(form.attributes[attribute.id]) }}</b></span></div><div class="stepper attribute-rank-stepper"><button type="button" @click="adjust(attribute.id,-1)">−</button><span>{{ form.attributes[attribute.id] }}</span><button type="button" @click="adjust(attribute.id,1)">+</button></div></div></article></div>
        </template>
        ...
        <template v-else-if="stepId==='path'">
          ...
          <aside class="creator-static-panel mana-path-panel winds-magic-panel"><strong>Winds of Magic</strong><p>The Winds of Magic are living currents that move through stone, storm, root, and heartbeat. Mana is the portion of that rhythm a hero can draw upon to fuel Abilities, Spells, and other supernatural effects.</p><div class="mana-stat-line"><span>Mana Pool <b>{{ manaPool }}</b></span><span>Restored at start of each round <b>{{ manaPerRound }}</b></span></div><p v-if="manaTraitNotes.length" class="mana-trait-note"><strong>Trait interaction:</strong> {{ manaTraitNotes.join(' · ') }}</p></aside>
        ...
        <template v-else>
          ...
          <details class="review-section review-collapsible character-sheet-attributes" open><summary><h2>Attributes</h2><span>Ranks &amp; derived stats</span></summary><div class="review-collapsible-body"><div class="review-attribute-cards single-row-attributes"><article v-for="attribute in attributes" :key="attribute.id"><strong>{{ attribute.name }}</strong><span>Rank {{ form.attributes[attribute.id] }}</span><small>Modifier +{{ attributeModifier(form.attributes[attribute.id]) }}</small></article></div><div class="review-secondary-strip"><article><span>Speed</span><strong>{{ derived.speed }}</strong></article><article><span>Aim</span><strong>{{ derived.aim }}</strong></article><article><span>Mettle</span><strong>{{ derived.mettle }}</strong></article><article><span>Fury</span><strong>{{ derived.fury }}</strong></article><article><span>Ward</span><strong>{{ derived.ward }}</strong></article><article><span>Guts</span><strong>{{ reviewGuts }}</strong><small v-if="gearGutsBonus">{{ derived.guts }} + {{ gearGutsBonus }} gear</small></article><article><span>Control</span><strong>{{ reviewControl }}</strong><small v-if="gearControlBonus">{{ derived.control }} + {{ gearControlBonus }} gear</small></article><article><span>Power</span><strong>{{ derived.power }}</strong></article><article><span>Heart</span><strong>{{ derived.heart }}</strong></article><article><span>Inspiration</span><strong>{{ derived.inspiration }}</strong></article><article class="review-mana-stat"><span>Mana Pool</span><small>Pool / Round</small><strong>{{ manaPool }} / +{{ manaPerRound }}</strong></article></div></div></details>
          ...
          <details v-if="form.path==='magic'" class="review-section review-collapsible"><summary><h2>Magic</h2></summary><div class="review-collapsible-body"><article class="review-feature-box lore full-rule-entry detail-tone-magic"><strong>Lore of {{ form.loreAttunement||'—' }}</strong><span>{{ loreDescription(form.loreAttunement)||'No Lore selected.' }}</span></article><h3 class="review-subheading">Lore Spells</h3><div class="review-spell-column"><article v-for="spell in reviewLoreSpells" :key="spell" class="spell-detail-card review-spell-detail-card" :class="spellLoreClass(spell)"><div class="spell-detail-head"><div><h2>{{ spell }} <small v-if="spell===signatureSpell" class="custom-content-badge">SIGNATURE</small><small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></h2></div><div class="trait-title-costs"><span v-for="type in abilityTypeKeywords(spellDetail(spell)?.keywords||[])" :key="type" class="ability-cost-pill">{{ type }}</span><span class="mana-badge"><template v-if="spellDetail(spell)?.manaCost!==null">{{ effectiveMana(spell) }} Mana</template><template v-else>Variable</template></span></div></div><p class="rule-flavor">{{ spellDetail(spell)?.flavor }}</p><div v-if="visibleRuleFields(spellDetail(spell)?.rules||'').length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cardKeywords(spellDetail(spell)?.keywords||[])" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article></div><h3 class="review-subheading">Invocation Spells</h3><div class="review-spell-column"><article v-for="spell in reviewInvocationSpells" :key="spell" class="spell-detail-card review-spell-detail-card" :class="spellLoreClass(spell)"><div class="spell-detail-head"><div><h2>{{ spell }} <small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></h2></div><div class="trait-title-costs"><span v-for="type in abilityTypeKeywords(spellDetail(spell)?.keywords||[])" :key="type" class="ability-cost-pill">{{ type }}</span><span class="mana-badge"><template v-if="spellDetail(spell)?.manaCost!==null">{{ effectiveMana(spell) }} Mana</template><template v-else>Variable</template></span></div></div><p class="rule-flavor">{{ spellDetail(spell)?.flavor }}</p><div v-if="visibleRuleFields(spellDetail(spell)?.rules||'').length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cardKeywords(spellDetail(spell)?.keywords||[])" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article><p v-if="!reviewInvocationSpells.length" class="muted">No Invocation Spells selected.</p></div></div></details>
          <details class="review-section review-collapsible"><summary><h2>Talents</h2></summary><div class="review-collapsible-body review-box-list review-talent-column"><article v-for="talent in form.talents.filter(Boolean)" :key="talent" class="review-feature-box talent full-rule-entry detail-tone-talent"><header><strong>{{ canonicalTalentName(talent) }}</strong><small>{{ talentCategory(talent) }}</small></header><p class="rule-flavor"><em>{{ structuredRule(talentText(talent)).intro }}</em></p><div v-if="visibleRuleFields(talentText(talent)).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(talentText(talent))" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div></article><p v-if="!form.talents.filter(Boolean).length">No Talents selected.</p></div></details>
...
.attribute-sheet-secondary>span{font-size:calc(9px + var(--font-offset))}
