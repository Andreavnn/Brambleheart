    ...
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
        ...
        <template v-else-if="stepId==='spark'">
          ...
          <details class="creation-info-panel help-panel" open><summary>What is a Spark &amp; Deed?</summary><div class="creation-info-body"><p>A Spark is the personality archetype that describes what tends to move your hero into action. Each Spark has two keywords that describe its nature.</p><p><strong>Deeds</strong> are meaningful acts completed during play — moments of courage, wit, compassion, discovery, or conviction that shape who your hero becomes. A completed Deed grants Experience based on its challenge and impact. If the Deed shares a keyword with your Spark, it grants <strong>+3 bonus XP</strong> in addition to the normal reward.</p><RouterLink to="/rules/read/sparks-deeds" class="inline-rule-link">Read Sparks &amp; Deeds →</RouterLink></div></details><article class="deed-example-card"><span class="eyebrow">EXAMPLE DEED</span><h2>Gentle Guardian</h2><p>Prevent or reduce harm to an ally through a save, healing, or an ability.</p><div class="deed-example-meta"><span><strong>Reward</strong> +1 XP</span><span><strong>Keywords</strong> Compassionate · Steadfast</span></div></article>
        ...
        <template v-else-if="stepId==='attributes'">
          ...
          <div class="attribute-editor attribute-sheet-editor"><article v-for="attribute in attributes" :key="attribute.id" class="attribute-sheet-row"><div class="attribute-sheet-copy"><div class="attribute-sheet-title"><strong>{{ attribute.name }}</strong><span class="attribute-sheet-modifier">Modifier <b>+{{ attributeModifier(form.attributes[attribute.id]) }}</b></span></div><small>{{ attribute.description }}</small><div class="attribute-sheet-secondary"><span v-for="secondary in secondaryStats[attribute.id]" :key="secondary.name"><RouterLink v-if="secondary.slug" :to="`/rules/read/${secondary.slug}`">{{ secondary.name }}</RouterLink><template v-else>{{ secondary.name }}</template><b>{{ secondary.name==='Speed'?derived.speed:secondary.name==='Aim'?derived.aim:secondary.name==='Mettle'?derived.mettle:secondary.name==='Power'?derived.power:secondary.name==='Ward'?derived.ward:secondary.name==='Guts'?derived.guts:derived.control }}</b></span><em v-if="!secondaryStats[attribute.id].length">No direct secondary stat</em></div></div><div class="attribute-sheet-rank attribute-rank-column"><small>Rank</small><div class="rank-stepper-line"><span class="max-marker">{{ form.attributes[attribute.id]===3?'MAX':'' }}</span><div class="stepper"><button type="button" @click="adjust(attribute.id,-1)">−</button><span>{{ form.attributes[attribute.id] }}</span><button type="button" @click="adjust(attribute.id,1)">+</button></div></div></div></article></div>
        </template>
        ...
        <template v-else-if="stepId==='equipment'">
          ...
          <section class="starting-equipment-panel"><div class="adventure-kit-bar"><div><strong>Adventure Kit (Starting Equipment Package)</strong><small>Starting Equipment</small></div><div class="kit-return-control"><span>Sell</span><label class="switch"><input v-model="form.adventureKit" type="checkbox" :true-value="false" :false-value="true"/><span></span></label></div></div><div v-if="form.adventureKit" class="adventure-kit-contents"><span>Bedroll &amp; Groundsheet</span><span>Traveler’s Cloak</span><span>2× Torches</span><span>Reed Flask</span><span>2× Trail Rations</span><span>Traveler’s Pack</span><span>Fire-Starting Kit</span></div><div class="wealth-balance-field"><span>Threadpiece Wealth</span><strong>{{ wealthRemaining }} NP remaining <small>of {{ startingWealth }} NP</small></strong><p>Starting Equipment &amp; Gear values use Nut Pieces (NP) in this creation pass. Costs and starting wealth have been reduced by roughly 25% to keep starting coin totals practical.</p></div></section>
