  ...
  <div class="app-page creator-page">
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
          ...
          <div class="species-choice-layout">
            <div class="species-choice-copy">
              ...
              <template v-if="selectedSpeciesData">
                ...
                <details class="creation-info-panel trait-panel culture-trait-panel"><summary>Culture Traits</summary><div class="creation-info-body"><p>Your Species begins with these two Culture Traits. In Step 2 you may keep them or exchange them for Culture Traits from other peoples.</p><div class="trait-stack"><article v-for="trait in selectedSpeciesData.cultureTraits" :key="trait.name" class="trait-card culture-trait-card"><div class="trait-card-head"><h3>{{ trait.name }} <small v-if="selectedSpeciesData.custom" class="custom-content-badge">CUSTOM</small></h3><div class="trait-title-costs"><span v-if="manaCostFromText(trait.text)!==null" class="mana-badge">{{ manaCostFromText(trait.text) }} Mana</span></div></div><p v-if="structuredRule(trait.text).intro" class="rule-flavor">{{ structuredRule(trait.text).intro }}</p><div v-if="visibleRuleFields(trait.text).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(trait.text)" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cultureFooterKeywords(trait.keywords,form.species,Boolean(cultureGrant(cultureId(form.species,trait.name))))" :key="keyword" :class="traitFooterClass(keyword)">{{ traitFooterLabel(keyword) }}</span></div></article></div></div></details>
              </template>
            </div>
          </div>
        </template>
        ...
        <template v-else>
          ...
          <details v-if="form.path==='magic'" class="review-section review-collapsible character-sheet-content-section"><summary><h2>Magic</h2></summary><div class="review-collapsible-body"><article class="review-feature-box lore full-rule-entry detail-tone-magic"><strong>Lore of {{ form.loreAttunement||'—' }}</strong><span>{{ loreDescription(form.loreAttunement)||'No Lore selected.' }}</span></article><h3 class="review-subheading">Lore Spells</h3><div class="spell-card-grid"><SpellCard v-for="spell in reviewLoreSpells" :key="spell" :title="spell" :cost="spellCostText(spell)" :lore="spellDetail(spell)?.lore||form.loreAttunement" :flavor="spellDetail(spell)?.flavor||''" :fields="visibleRuleFields(spellDetail(spell)?.rules||'')" :keywords="spellDetail(spell)?.keywords||[]" :custom="isCustomSpell(spell)" :signature="spell===signatureSpell" /></div><h3 class="review-subheading">Invocation Spells</h3><div class="spell-card-grid"><SpellCard v-for="spell in reviewInvocationSpells" :key="spell" :title="spell" :cost="spellCostText(spell)" :lore="spellDetail(spell)?.lore||'Invocation'" :flavor="spellDetail(spell)?.flavor||''" :fields="visibleRuleFields(spellDetail(spell)?.rules||'')" :keywords="spellDetail(spell)?.keywords||[]" :custom="isCustomSpell(spell)" /><p v-if="!reviewInvocationSpells.length" class="muted">No Invocation Spells selected.</p></div></div></details>
          <details class="review-section review-collapsible character-sheet-content-section"><summary><h2>Talents</h2></summary><div class="review-collapsible-body review-box-list review-talent-column"><article v-for="talent in form.talents.filter(Boolean)" :key="talent" class="review-feature-box talent full-rule-entry detail-tone-talent" :class="talentCardClasses(talent)"><header><strong>{{ canonicalTalentName(talent) }}</strong></header><p class="rule-flavor"><em>{{ structuredRule(talentText(talent)).intro }}</em></p><div v-if="visibleRuleFields(talentText(talent)).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(talentText(talent))" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in talentFeaturePillKeywords(talent,talentKeywords(talent))" :key="keyword" :class="abilityFeaturePillClass(keyword)">{{ abilityFeaturePillLabel(keyword) }}</span></div></article><p v-if="!form.talents.filter(Boolean).length">No Talents selected.</p></div></details>
          <details class="review-section review-collapsible character-sheet-content-section"><summary><h2>Equipment &amp; Gear</h2><span>{{ threadpieceBalanceLabel }} — {{ carriedWeightLabel }}</span></summary><div class="review-collapsible-body"><div v-if="reviewEquipmentItems.length" class="review-equipment-grid"><article v-for="(item,index) in reviewEquipmentItems" :key="`${item.name}-${index}`"><strong>{{ item.name }}<template v-if="equipmentQuantity(item)>1"> ×{{ equipmentQuantity(item) }}</template></strong><small><template v-if="item.category!=='Adventure Kit'">{{ item.category }} · </template>{{ formatGearCost(item) }}<template v-if="isStackablePurchased(item)"> each</template></small><p v-if="item.category!=='Adventure Kit'&&'detail' in item&&item.detail">{{ item.detail }}</p><p v-if="item.effect">{{ item.effect }}</p><small v-if="item.choice">Choice: {{ item.choice }}</small><small v-if="item.attachedTo">Attached to: {{ item.attachedTo }}</small></article></div><p v-else class="muted">No additional equipment purchased.</p></div></details>
    ...
    <div v-if="talentPickerOpen" class="modal-backdrop talent-picker-backdrop" @click.self="talentPickerOpen=false">
      <section ref="talentDialog" class="modal-card talent-picker-modal" role="dialog" aria-modal="true" aria-label="Choose a Talent">
        ...
        <div class="talent-picker-list"><article v-for="talent in filteredTalentPicker" :key="talent" class="talent-detail-card talent-picker-card" :class="[...talentCardClasses(talent),{invalid:!talentRequirementMet(talent)}]"><div class="talent-detail-head"><div><span v-if="isCustomTalent(talent)" class="custom-content-badge">CUSTOM</span><h2>{{ canonicalTalentName(talent) }}</h2></div><div class="talent-head-actions"><span v-if="manaCostFromText(talentText(talent))!==null" class="mana-badge">{{ manaCostFromText(talentText(talent)) }} Mana</span><span v-if="talentRequires(talent)" class="requirement-badge" :class="{invalid:!talentRequirementMet(talent)}">Requires {{ talentRequires(talent) }}</span></div></div><p v-if="structuredRule(talentText(talent)).intro" class="rule-flavor">{{ structuredRule(talentText(talent)).intro }}</p><div v-if="visibleRuleFields(talentText(talent)).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(talentText(talent))" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in talentFeaturePillKeywords(talent,talentKeywords(talent))" :key="keyword" :class="abilityFeaturePillClass(keyword)">{{ abilityFeaturePillLabel(keyword) }}</span></div><div class="talent-picker-action"><button type="button" class="secondary-button" :class="{active:talentSelected(talent)}" :disabled="!talentSelected(talent)&&(form.talents.filter(Boolean).length>=requiredTalentCount()||!talentRequirementMet(talent))" @click="toggleTalentChoice(talent)">{{ talentSelected(talent)?'Selected':'Select' }}</button></div></article></div><div class="talent-picker-footer"><span>{{ form.talents.filter(Boolean).length }} / {{ requiredTalentCount() }} selected</span><button type="button" class="primary-button" :disabled="!talentsValid()" @click="talentPickerOpen=false">Done</button></div>
      </section>
    </div>
