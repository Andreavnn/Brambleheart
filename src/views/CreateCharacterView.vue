...
const {creationTips}=useSettings()
    ...
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
          ...
          <div class="species-choice-layout">
            <div class="species-choice-copy">
              ...
              <template v-if="selectedSpeciesData">
                <details class="creation-info-panel species-lore-panel" :open="creationTips"><summary>{{ selectedSpeciesData.name }} Lore <span v-if="selectedSpeciesData.custom" class="custom-content-badge">CUSTOM</span></summary><div class="creation-info-body"><p class="species-quote">“{{ selectedSpeciesData.quote }}”</p><p>{{ selectedSpeciesData.lore }}</p><div class="keyword-pill-row"><span class="keyword-pill">{{ selectedSpeciesData.theme }}</span><span class="keyword-pill">{{ selectedSpeciesData.language.split(',')[0] }}</span></div></div></details>
        ...
        <template v-else-if="stepId==='skills'">
          ...
          <details class="creation-info-panel help-panel" :open="creationTips"><summary>How do Starting Skills work?</summary><div class="creation-info-body"><p>The supplied Character Creation rules grant two Skills from your Homeland at Rank 1. There is no separate pool of Skill points to spend during creation. Some selected Culture Traits grant additional Skill ranks; those are added automatically below and can raise a Skill already gained from your Homeland.</p><RouterLink to="/rules/read/attributes-skills" class="inline-rule-link">Read Attributes &amp; Skills →</RouterLink></div></details>
        ...
        <template v-else-if="stepId==='path'">
          ...
          <details class="creation-info-panel help-panel" :open="creationTips"><summary>Choose Your Path</summary><div class="creation-info-body"><p>This choice decides how your hero first expresses exceptional ability. Every path grants at least one Talent. Wind-Touched begins magical progression, Gifted Heart begins with a second Talent, Practiced Hand broadens Skills, and Tempered Form strengthens one Attribute.</p></div></details>
        ...
        <template v-else-if="stepId==='path-attribute'">
          ...
          <details class="creation-info-panel help-panel" :open="creationTips"><summary>Choose One Attribute</summary><div class="creation-info-body"><p>Tempered Form strengthens one Attribute before you choose your Talent.</p></div></details>
        ...
        <template v-else-if="stepId==='talents'">
          ...
          <details class="creation-info-panel help-panel" :open="creationTips"><summary>What are Talents?</summary><div class="creation-info-body"><p>Talents represent trained techniques, unusual gifts, and practiced capabilities. Some Talents require another Talent first. Requirements must be met before this character can be finished.</p></div></details>
        ...
        <template v-else>
          ...
          <details class="review-section review-collapsible character-sheet-content-section"><summary><h2>Talents</h2></summary><div class="review-collapsible-body review-box-list review-talent-column"><article v-for="talent in form.talents.filter(Boolean)" :key="talent" class="review-feature-box talent full-rule-entry detail-tone-talent" :class="talentCardClasses(talent)"><header><strong>{{ canonicalTalentName(talent) }}</strong></header><p class="rule-flavor"><em>{{ structuredRule(talentText(talent)).intro }}</em></p><div v-if="visibleRuleFields(talentText(talent)).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(talentText(talent))" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in abilityFeaturePillKeywords(talentKeywords(talent))" :key="keyword" :class="abilityFeaturePillClass(keyword)">{{ abilityFeaturePillLabel(keyword) }}</span></div></article><p v-if="!form.talents.filter(Boolean).length">No Talents selected.</p></div></details>
