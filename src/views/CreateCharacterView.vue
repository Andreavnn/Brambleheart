    ...
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
        ...
        <template v-else-if="stepId==='spells'">
          ...
          <div class="spell-card-grid selected-spell-stack"><SpellCard v-for="spell in [...form.spells,...form.invocationSpells].filter(Boolean)" :key="spell" :title="spell" :cost="spellCostText(spell)" :lore="spellDetail(spell)?.lore||'Invocation'" :flavor="spellDetail(spell)?.flavor||''" :fields="visibleRuleFields(spellDetail(spell)?.rules||'')" :keywords="spellDetail(spell)?.keywords||[]" :custom="isCustomSpell(spell)" :signature="spell===signatureSpell" /></div>
        </template>
        ...
        <template v-else>
          ...
          <details class="review-section review-collapsible character-sheet-content-section"><summary><h2>Equipment &amp; Gear</h2><span>{{ threadpieceBalanceLabel }} — {{ carriedWeightLabel }}</span></summary><div class="review-collapsible-body"><div v-if="reviewEquipmentCards.length" class="review-equipment-grid character-equipment-card-grid"><EquipmentCard v-for="(item,index) in reviewEquipmentCards" :key="`${item.name}-${index}`" :title="item.name" :badge="item.costText||''" :tone-class="equipmentToneClass(item)" :fields="equipmentCardFields(item)" :description="item.description||''"><template #footer><div class="character-equipment-card-footer"><small v-if="equipmentQuantity(item)>1">Quantity: {{ equipmentQuantity(item) }}</small><small v-if="item.choice">Choice: {{ item.choice }}</small><small v-if="item.attachedTo">Attached to: {{ item.attachedTo }}</small></div></template></EquipmentCard></div><p v-else class="muted">No additional equipment purchased.</p></div></details>
