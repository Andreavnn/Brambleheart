...
import { ABILITY_TYPE_KEYWORDS, abilityPillLabel, abilityTypeClass, activeAbilityKeywords, canonicalAbilityType } from '../rules/abilityPresentation'
...
function singleKeywords(values:string[]|undefined){return activeAbilityKeywords(values)}
function abilityTypeKeywords(values:string[]|undefined){return Array.from(new Set(singleKeywords(values).map(canonicalAbilityType).filter(value=>ABILITY_TYPE_KEYWORDS.has(value))))}
...
function traitFooterKeywords(values:string[]|undefined,traitType='',speciesName=''){return Array.from(new Set(['Ability',...abilityTypeKeywords(values),...cardKeywords(values,traitType,speciesName)]))}
function traitFooterClass(keyword:string){if(keyword==='Ability')return['ability-cost-pill','type-ability'];return abilityTypeKeywords([keyword]).length?['ability-cost-pill',abilityTypeClass(keyword)]:['keyword-pill']}
function traitFooterLabel(keyword:string){return keyword==='Ability'?'ABILITY':abilityTypeKeywords([keyword]).length?abilityPillLabel(keyword):canonicalAbilityType(keyword)}
    ...
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
        ...
        <template v-else-if="stepId==='lore'">
          ...
          <template v-if="form.loreAttunement"><article class="choice-summary lore-fluff-card"><h2>Lore of {{ form.loreAttunement }}</h2><p>{{ loreDescription(form.loreAttunement) }}</p></article><article v-if="signatureDetail" class="spell-detail-card signature-spell-card" :class="spellLoreClass(signatureDetail.name)"><div class="spell-detail-head"><div><span class="eyebrow">SIGNATURE SPELL</span><h2>{{ signatureDetail.name }}</h2></div><div class="trait-title-costs"><span class="ability-cost-pill type-ability">ABILITY</span><span v-for="type in abilityTypeKeywords(signatureDetail.keywords)" :key="type" :class="['ability-cost-pill',abilityTypeClass(type)]">{{ abilityPillLabel(type) }}</span><span class="mana-badge">{{ spellCostText(signatureDetail.name) }}</span></div></div><p class="rule-flavor">{{ signatureDetail.flavor }}</p><div class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(signatureDetail.rules)" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cardKeywords(signatureDetail.keywords)" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article></template>
        </template>
        ...
        <template v-else-if="stepId==='spells'">
          ...
          <div class="selected-spell-stack"><article v-for="spell in [...form.spells,...form.invocationSpells].filter(Boolean)" :key="spell" class="spell-detail-card" :class="spellLoreClass(spell)"><div class="spell-detail-head"><div><h2>{{ spell }} <small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></h2></div><div class="trait-title-costs"><span class="ability-cost-pill type-ability">ABILITY</span><span v-for="type in abilityTypeKeywords(spellDetail(spell)?.keywords||[])" :key="type" :class="['ability-cost-pill',abilityTypeClass(type)]">{{ abilityPillLabel(type) }}</span><span class="mana-badge">{{ spellCostText(spell) }}</span></div></div><p class="rule-flavor">{{ spellDetail(spell)?.flavor }}</p><div class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cardKeywords(spellDetail(spell)?.keywords||[])" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article></div>
        </template>
      ...
      <section ref="cultureDialog" class="modal-card culture-picker-modal" role="dialog" aria-modal="true" aria-label="Choose Culture Traits">
        ...
        <div class="culture-picker-list">
          <article v-for="trait in filteredCultureTraits" :key="`${trait.species}-${trait.name}`" class="culture-picker-row">
            ...
            <div class="keyword-pill-row"><span v-for="keyword in cultureFooterKeywords(trait.keywords,'Cultural',trait.species)" :key="keyword" :class="traitFooterClass(keyword)">{{ traitFooterLabel(keyword) }}</span></div>
          </article>
        </div>
