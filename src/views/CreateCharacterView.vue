...
function talentCategory(name:string){
  ...
  if(custom&&custom.category!=='Combat')return custom.category
...
function resetForm(){
  ...
  form.spark=sparks[0][0];form.homeland='';form.customHomelandName='';form.customHomelandDetail='';form.skills=['',''];form.faith='';form.oath='';
    ...
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
        <template v-if="stepId==='species'">
          ...
          <div class="species-choice-layout">
            <div class="species-choice-copy">
              <label class="field-label">Species<select v-model="form.species" class="field-control"><option value="">Choose Your Species</option><option v-for="item in availableSpecies" :key="item" :value="item">{{ item }}{{ customSpecies(item)?' (Custom)':'' }}</option></select></label>
        ...
        <template v-else-if="stepId==='faith-oath'">
          ...
          <div class="faith-oath-column"><section><details class="creation-info-panel help-panel" open><summary>What is an Oath?</summary><div class="creation-info-body"><p>An Oath is the principle your hero has chosen to live by. It does not provide a flat mechanical bonus, but keeping or breaking it can shape reputation, narrative consequences, and how your character is remembered.</p></div></details><label class="field-label faith-oath-select">Oath<select v-model="form.oath" class="field-control"><option value="">Select Oath</option><option v-for="item in oaths" :key="item[0]" :value="item[0]">{{ item[0] }} — {{ item[1] }}</option></select></label><article v-if="form.oath" class="choice-summary"><h2>{{ form.oath }}</h2><p>{{ oathDescription }}</p></article></section><section><details class="creation-info-panel help-panel" open><summary>What is Faith?</summary><div class="creation-info-body"><p>Faith describes what your hero reveres, trusts, or believes gives meaning to the world. It helps frame decisions and relationships without forcing one way to play the character.</p></div></details><label class="field-label faith-oath-select">Faith<select v-model="form.faith" class="field-control"><option value="">Select Faith</option><option v-for="item in faiths" :key="item">{{ item }}</option></select></label><article v-if="form.faith" class="choice-summary"><h2>{{ form.faith }}</h2><p>{{ faithDescription }}</p></article></section></div>
        </template>
