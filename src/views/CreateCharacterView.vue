    ...
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
        ...
        <template v-else-if="stepId==='spells'">
          ...
          <div class="selected-spell-stack"><article v-for="spell in [...form.spells,...form.invocationSpells].filter(Boolean)" :key="spell" class="spell-detail-card" :class="spellLoreClass(spell)"><div class="spell-detail-head"><div><h2>{{ spell }} <small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></h2></div><div class="trait-title-costs"><span class="mana-badge">{{ spellCostText(spell) }}</span></div></div><p class="rule-flavor">{{ spellDetail(spell)?.flavor }}</p><div class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in abilityFeaturePillKeywords(spellDetail(spell)?.keywords||[])" :key="keyword" :class="abilityFeaturePillClass(keyword)">{{ abilityFeaturePillLabel(keyword) }}</span></div></article></div>
        </template>
        ...
        <template v-else>
          ...
          <details v-if="form.path==='magic'" class="review-section review-collapsible character-sheet-content-section"><summary><h2>Magic</h2></summary><div class="review-collapsible-body"><article class="review-feature-box lore full-rule-entry detail-tone-magic"><strong>Lore of {{ form.loreAttunement||'—' }}</strong><span>{{ loreDescription(form.loreAttunement)||'No Lore selected.' }}</span></article><h3 class="review-subheading">Lore Spells</h3><div class="review-spell-column"><article v-for="spell in reviewLoreSpells" :key="spell" class="spell-detail-card review-spell-detail-card" :class="spellLoreClass(spell)"><div class="spell-detail-head"><div><h2>{{ spell }} <small v-if="spell===signatureSpell" class="custom-content-badge">SIGNATURE</small><small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></h2></div><div class="trait-title-costs"><span class="mana-badge">{{ spellCostText(spell) }}</span></div></div><p class="rule-flavor">{{ spellDetail(spell)?.flavor }}</p><div v-if="visibleRuleFields(spellDetail(spell)?.rules||'').length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in abilityFeaturePillKeywords(spellDetail(spell)?.keywords||[])" :key="keyword" :class="abilityFeaturePillClass(keyword)">{{ abilityFeaturePillLabel(keyword) }}</span></div></article></div><h3 class="review-subheading">Invocation Spells</h3><div class="review-spell-column"><article v-for="spell in reviewInvocationSpells" :key="spell" class="spell-detail-card review-spell-detail-card" :class="spellLoreClass(spell)"><div class="spell-detail-head"><div><h2>{{ spell }} <small v-if="isCustomSpell(spell)" class="custom-content-badge">CUSTOM</small></h2></div><div class="trait-title-costs"><span class="mana-badge">{{ spellCostText(spell) }}</span></div></div><p class="rule-flavor">{{ spellDetail(spell)?.flavor }}</p><div v-if="visibleRuleFields(spellDetail(spell)?.rules||'').length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(spellDetail(spell)?.rules||'')" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in abilityFeaturePillKeywords(spellDetail(spell)?.keywords||[])" :key="keyword" :class="abilityFeaturePillClass(keyword)">{{ abilityFeaturePillLabel(keyword) }}</span></div></article><p v-if="!reviewInvocationSpells.length" class="muted">No Invocation Spells selected.</p></div></div></details>
