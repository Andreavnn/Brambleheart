...
function formatGearCost(item:Pick<PurchasedEquipment,'costSp'|'costNp'>){
  const np=Math.max(0,Number(item.costNp??gearCostNp(item)))
  const wp=Math.round(np*10)
  return wp%10===0?`${wp/10} NP`:`${wp} WP`
}
