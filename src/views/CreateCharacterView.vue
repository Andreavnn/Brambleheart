...
const STARTING_WEALTH_NP=250
const ADVENTURE_KIT_SELL_NP=15
const ADVENTURE_KIT_WEIGHT_LB=15
function gearCostNp(item:{costSp:number}){return Math.max(0,Number(item.costSp||0)*5)}
...
function formatGearCost(item:Pick<PurchasedEquipment,'costSp'>){
  const wp=Math.round(Number(item.costSp||0)*50)
  if(wp%250===0&&wp>=250)return`${wp/250} BP`
  if(wp%50===0&&wp>=50)return`${wp/50} SP`
  if(wp%10===0&&wp>=10)return`${wp/10} NP`
  return`${wp} WP`
}
...
function homelandRaisedExistingSkill(skill:string){return homelandRankForSkill(skill)>0&&cultureRankForSkill(skill)>0}
...
function gearShopSubtitle(item:typeof gearShopItems[number]){const cost=item.costText;return gearTab.value==='All'?`${gearTabFor(item)} · ${cost}`:cost}
...
function addEquipment(item:typeof gearShopItems[number]){
  ...
  form.equipment.push({name:item.name,costSp:item.costSp,costNp,category:item.category,detail:gearDisplayDetail(item),effect:gearEffect(item),choice:choice||undefined,quantity:1})
}
...
function reviewWeaponProfile(item:PurchasedEquipment|undefined){
  if(!item)return weaponProfile(undefined)
  const source=sourceGearFor(item)
  const detail=source&&source.category==='Weapon'?gearDisplayDetail(source):item.detail
  const base=weaponProfile({...item,detail})
  const hasKnot=journeyKnotAttachedTo(item.name)
  const properties=[base.properties==='—'?'':base.properties,hasKnot?'Journey Knot':''].filter(Boolean).join(', ')||'—'
  return{...base,damage:addDamageBonus(base.damage,hasKnot?1:0),properties}
}
...
const carriedWeight=computed(()=>form.equipment.reduce((sum,item)=>sum+parseWeight(item.detail)*equipmentQuantity(item),form.adventureKit?ADVENTURE_KIT_WEIGHT_LB:0))
