...
import { armorProfile, derivedStats, equipmentGutsBonus, equipmentManaSyphon, normalizeSkillName, rankModifier, structuredRule, visibleRuleFields, weaponProfile } from '../rules/rulesEngine'
...
const customHomelandSkillPools=computed(()=>{
  ...
  // Supplied Homeland rules: one Skill from Lore/Might/Prowess and one from Courage/Lore.
  // Current skill trees use Agility for Prowess and Bravery for Courage.
...
function cultureTitleKeywords(values:string[]|undefined){return abilityTypeKeywords(values).filter(value=>value.toLowerCase()!=='passive')}
function cultureFooterKeywords(values:string[]|undefined,traitType='',speciesName=''){
  const footer=cardKeywords(values,traitType,speciesName)
  if(singleKeywords(values).some(value=>value.toLowerCase()==='passive'))footer.unshift('Passive')
  return Array.from(new Set(footer))
}
...
function canAddEquipment(item:typeof gearShopItems[number]){
  return adjustedGearCostNp(item)<=wealthRemaining.value && (!gearChoices(item).length||Boolean(gearChoice.value[item.name])) && form.attributes.might>=gearMightRequirement(item)
}
...
function gearDisplayDetail(item:typeof gearShopItems[number]){
  ...
  if(item.category==='Weapon'){
    const parts=item.detail.split('·').map(part=>part.trim())
    if(parts.length>=2)return `Damage ${parts[0]} · Weight ${parts[1]}`
  }
...
function choosePath(path:'magic'|'talents'){setPath(path);next()}
...
function addEquipment(item:typeof gearShopItems[number]){if(!canAddEquipment(item))return;const costNp=adjustedGearCostNp(item);const choice=gearChoice.value[item.name]||'';form.equipment.push({name:item.name,costSp:item.costSp,costNp,category:item.category,detail:gearDisplayDetail(item),effect:gearEffect(item),choice:choice||undefined})}
...
function effectiveMana(name:string){
  ...
  return Math.max(0,detail.manaCost-(detail.lore===form.loreAttunement?2:0)+gearManaSyphon.value)
}
...
function resetForm(){
  ...
  draftId.value=null;originalCreatedAt.value=null;originalStatus.value='incomplete';reviewNameLocked.value=true;reviewCampaignLocked.value=true;stepIndex.value=0;error.value='';window.scrollTo({top:0,behavior:'smooth'})
}
    ...
    <main class="page character-create-page creator-reader-page">
      ...
      <section class="form-card creation-step-card">
          ...
          <div class="species-choice-layout">
            <div class="species-choice-copy">
              ...
              <template v-if="selectedSpeciesData">
                ...
                <details class="creation-info-panel trait-panel culture-trait-panel"><summary>Culture Traits</summary><div class="creation-info-body"><p>Your Species begins with these two Culture Traits. In Step 2 you may keep them or exchange them for Culture Traits from other peoples.</p><div class="trait-stack"><article v-for="trait in selectedSpeciesData.cultureTraits" :key="trait.name" class="trait-card culture-trait-card"><div class="trait-card-head"><h3>{{ trait.name }} <small v-if="selectedSpeciesData.custom" class="custom-content-badge">CUSTOM</small></h3><div class="trait-title-costs"><span v-for="type in cultureTitleKeywords(trait.keywords)" :key="type" class="ability-cost-pill">{{ type }}</span><span v-if="manaCostFromText(trait.text)!==null" class="mana-badge">{{ manaCostFromText(trait.text) }} Mana</span></div></div><p v-if="structuredRule(trait.text).intro" class="rule-flavor">{{ structuredRule(trait.text).intro }}</p><div v-if="visibleRuleFields(trait.text).length" class="rule-breakdown-grid"><div v-for="field in visibleRuleFields(trait.text)" :key="field.label"><small>{{ field.label }}</small><span>{{ field.value }}</span></div></div><div class="keyword-pill-row"><span v-for="keyword in cultureFooterKeywords(trait.keywords,'Cultural',form.species)" :key="keyword" class="keyword-pill">{{ keyword }}</span></div></article></div></div></details>
              </template>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="stepId==='attributes'">
          ...
          <div class="attribute-editor attribute-sheet-editor"><article v-for="attribute in attributes" :key="attribute.id" class="attribute-sheet-row"><div class="attribute-sheet-copy"><div class="attribute-sheet-title"><strong>{{ attribute.name }}</strong></div><small>{{ attribute.description }}</small></div><div class="attribute-rank-stack"><div class="attribute-rank-mod-row"><span class="attribute-rank-display"><small>Rank</small><b>{{ form.attributes[attribute.id] }}</b></span><span class="attribute-mod-display"><small>Modifier</small><b>+{{ attributeModifier(form.attributes[attribute.id]) }}</b></span></div><div class="stepper"><button type="button" @click="adjust(attribute.id,-1)">−</button><span>{{ form.attributes[attribute.id] }}</span><button type="button" @click="adjust(attribute.id,1)">+</button></div></div><div class="attribute-sheet-secondary"><span v-for="secondary in secondaryStats[attribute.id]" :key="secondary.name"><RouterLink v-if="secondary.slug" :to="`/rules/read/${secondary.slug}`">{{ secondary.name }}</RouterLink><template v-else>{{ secondary.name }}</template><b>{{ secondary.name==='Speed'?derived.speed:secondary.name==='Aim'?derived.aim:secondary.name==='Mettle'?derived.mettle:secondary.name==='Power'?derived.power:secondary.name==='Ward'?derived.ward:secondary.name==='Guts'?derived.guts:derived.control }}</b></span><em v-if="!secondaryStats[attribute.id].length">No direct secondary stat</em></div></article></div>
        </template>
      ...
      <section ref="cultureDialog" class="modal-card culture-picker-modal" role="dialog" aria-modal="true" aria-label="Choose Culture Traits">
        ...
        <div class="culture-picker-list">
          <article v-for="trait in filteredCultureTraits" :key="`${trait.species}-${trait.name}`" class="culture-picker-row">
            ...
            <div class="keyword-pill-row"><span v-for="keyword in cultureFooterKeywords(trait.keywords,'Cultural',trait.species)" :key="keyword" class="keyword-pill">{{ keyword }}</span></div>
          </article>
        </div>
