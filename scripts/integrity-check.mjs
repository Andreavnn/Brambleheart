...
assert.deepEqual(changelogReleases,['0.03','0.02','0.01'],'Site Update history must retain the condensed three-release history')
...
assert.match(ruleReader,/<h2>Known Spells<\/h2>[\s\S]{0,1800}<div class="spell-card-grid lore-spell-card-grid">[\s\S]{0,300}<SpellCard[\s\S]{0,300}v-for="spell in currentLoreSpells"/,'Lore pages must render Known Spells through the shared SpellCard authority in a single-column grid')
...
assert.match(simulatorView,/<RuleFeatureCard[\s\S]{0,900}otherGear/,'Character Sheet carried equipment must reuse RuleFeatureCard presentation')
...
assert.match(ruleReader,/range-area-grid\{[^}]*grid-template-columns:repeat\(3,44px\)[^}]*gap:5px/,'Spell Range Orb cells must align over the square path while remaining a 3x3 grid')
