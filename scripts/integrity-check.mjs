...
assert.deepEqual(changelogReleases,['0.10','0.09','0.08','0.07','0.06','0.05','0.04','0.03','0.02','0.01'],'Site Update history must retain the condensed history and advance normally')
...
assert.match(ruleReader,/<h2>Known Spells<\/h2>[\s\S]{0,1800}<SpellCard[\s\S]{0,300}v-for="spell in currentLoreSpells"/,'Lore pages must render the established Known Spells card grid')
...
assert.match(styles,/\.spell-card-grid\{display:grid;grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/,'Spell cards must restore the two-column desktop layout')
