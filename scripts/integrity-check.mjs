...
assert.deepEqual(changelogReleases,['0.06','0.05','0.04','0.03','0.02','0.01'],'Site Update history must retain the condensed history and advance normally')
...
assert.equal((gameUpdatesSource.match(/\n  \{\n    version:/g)||[]).length,2,'Game Update history must contain current v0.02 plus the consolidated v0.01 Launch Patch')
assert.match(gameUpdatesSource,/version:'0\.01'[\s\S]{0,100}title:'Launch Patch'/,'Consolidated Launch Patch must remain the prior Game Update')
assert.match(gameUpdatesSource,/title:'Bound Equipment'/,'Current Game Update must document bound equipment rules')
...
assert.match(charOptions,/"name": "Scriptweave Book"[\s\S]{0,900}one known Lore Spell[\s\S]{0,500}COOLDOWN: \[1d10\/2\+1\]/,'Scriptweave Book must bind one Lore Spell and add the current cooldown')
...
assert.match(simulatorView,/effectiveRhythmConditions[\s\S]{0,6000}selectedHealthState\.value\.condition|selectedHealthState\.value\.condition[\s\S]{0,6000}effectiveRhythmConditions/,'Dice Roller must apply the selected character Health Condition')
assert.match(simulatorView,/Threat Level · Point/,'Character Sheet Threat card must use the requested label')
assert.match(simulatorView,/Equipment &amp; Gear - \{\{ equipmentWealthLabel \}\}/,'Equipment heading must show current Threadpieces')
