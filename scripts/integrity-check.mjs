...
assert.ok(styles.includes('.spell-detail-card,.review-feature-box.spell,.spell-collapsible{--rule-tone:var(--spell-lore,var(--detail-magic));}'),'all spell card forms must inherit the canonical Lore tone')
assert.ok(styles.includes('.semantic-collapsible{background:color-mix(in srgb,var(--rule-tone,var(--accent)) 11%,var(--surface-front))'),'semantic collapsible menus must preserve their assigned accent surface')
assert.match(ruleReader,/semantic-collapsible spell-collapsible card-surface[^>]*:class="spellLoreClass\(spell\)"/,'Rules Reader collapsible spell boxes must retain Lore accent styling')
for(const toneClass of ['detail-tone-talent','detail-tone-spark','detail-tone-deed','detail-tone-equipment'])assert.match(ruleReader,new RegExp(`semantic-collapsible[^"\n]*${toneClass}|${toneClass}[^"\n]*semantic-collapsible`),`Rules Reader semantic menu missing ${toneClass} accent`)
assert.match(ruleReader,/monster-profile-card\{[^}]*color-mix\(in srgb,var\(--rule-tone,var\(--accent\)\)/,'Monster action cards must tint from their canonical action tone')
...
assert.match(settingsState,/saved\.menusExpanded\?\?saved\.creationTips/,'Legacy Tips state must migrate only at the Settings boundary')
