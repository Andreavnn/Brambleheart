...
assert.deepEqual(changelogReleases,['0.04','0.03','0.02'],'Site Update history must retain the condensed three-release history')
...
assert.match(ruleReader,/<h2>Known Spells<\/h2>[\s\S]{0,2200}currentLoreSignatureSpell[\s\S]{0,1200}<RuleCollapsibleCard title="Lore Spells"[\s\S]{0,900}<div v-if="currentLoreKnownSpells.length" class="spell-card-grid lore-spell-card-grid">[\s\S]{0,400}<SpellCard[\s\S]{0,300}v-for="spell in currentLoreKnownSpells"/,'Lore pages must render Signature and Lore Spells through the shared SpellCard authority in a single-column layout')
...
assert.match(styles,/\.lore-spell-card-grid\{grid-template-columns:1fr;\}/,'Lore pages must force the shared Spell card grid to one column')
