...
assert.deepEqual(changelogReleases,['0.07','0.06','0.05','0.04','0.03','0.02','0.01'],'Site Update history must retain the condensed history and advance normally')
...
assert.match(changelog,/## Game Changes[\s\S]{0,1600}Sea of Roots[\s\S]{0,1000}Driftwood Charm/,'Beta 0.05 game-rule changes must be categorized as Game Changes')
assert.match(changelog,/## Game Changes[\s\S]{0,1800}7–10 XP[\s\S]{0,900}Tordan Steady Pace/,'Progression and Species tuning must be categorized as Game Changes')
...
assert.match(simulatorView,/grid-template-columns:repeat\(10,minmax\(0,1fr\)\)/,'Character Sheet Mana tracker must use ten cells per row')
...
assert.match(ruleReader,/monster-profile-section[\s\S]{0,5000}Actions[\s\S]{0,3000}Special Rules[\s\S]{0,3000}Keywords/,'Monster profile pages must expose Actions, Special Rules, and Keywords')
